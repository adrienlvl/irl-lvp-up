...
app.whenReady().then(async () => {
  ...
  try {
    ...
    const checks = await win.webContents.executeJavaScript(`(async function(){
      ...
      const checks = {
        ...
        attentionDigest: typeof attentionDigest === 'function' && !!document.getElementById('attentionDigest') && !!document.getElementById('attentionPanel') && (() => {
          ...
          // Sommeil URGENT (court + coucher en dents de scie) remonte en 'readiness' high même quand
          // readiness ≥ 50 (le n°1 santé d'Adrien n'est plus invisible hors onglet Récupération).
          const urgentSleep = { recovery: [
            ...
            { date: '2026-07-21', sleep: 5.2, bedtime: '02:40', energy: 3, hrv: 60, soreness: 2 },
          ], agenda: [], workouts: [], habits: [] };
          const us = attentionDigest(urgentSleep, '2026-07-21');
        ...
        keyDateMarkers: typeof keyDateMarkers === 'function' && keyDateMarkers({ title: 'BTS', date: '2026-05-15' }, { date: '2026-06-01' }, '2026-05-15')[0].kind === 'exam' && keyDateMarkers(null, null, '2026-05-15').length === 0 && keyDateMarkers([{ title: 'Droit', date: '2026-05-15' }, { title: 'Compta', date: '2026-05-15' }], null, '2026-05-15').length === 2,
        upcomingDeadlines: typeof upcomingKeyDates === 'function' && !!document.getElementById('upcomingDeadlines') && (r => r.length === 2 && r[0].kind === 'race')(upcomingKeyDates({ date: '2026-07-25' }, { date: '2026-07-15' }, '2026-07-10', 60)) && (r => r.length === 2 && r[0].label === 'Compta' && r[1].label === 'Droit')(upcomingKeyDates([{ title: 'Droit', date: '2026-07-25' }, { title: 'Compta', date: '2026-07-12' }], null, '2026-07-10', 60)),
        ...
        weeklyInsights: typeof weeklyInsights === 'function' && !!document.getElementById('weeklyInsights') && (() => { const ins = weeklyInsights({ workouts: [{ date: '2026-07-06', type: 'strength', duration: 50, effort: 3 }], goals: { sessions: 4 } }, '2026-07-06', '2026-07-11'); if (!(Array.isArray(ins) && ins.length >= 1 && ins.every(i => i.emoji && i.text && i.tone))) return false; const spikeW = [{ date: '2026-06-23', type: 'muscu', duration: 30, effort: 2 }, { date: '2026-06-26', type: 'muscu', duration: 30, effort: 2 }, { date: '2026-06-30', type: 'muscu', duration: 30, effort: 2 }, { date: '2026-07-03', type: 'muscu', duration: 35, effort: 2 }, { date: '2026-07-07', type: 'muscu', duration: 35, effort: 2 }, { date: '2026-07-09', type: 'muscu', duration: 35, effort: 2 }, { date: '2026-07-13', type: 'muscu', duration: 60, effort: 3 }, { date: '2026-07-14', type: 'muscu', duration: 60, effort: 3 }, { date: '2026-07-15', type: 'muscu', duration: 60, effort: 3 }, { date: '2026-07-16', type: 'muscu', duration: 60, effort: 3 }]; const si = weeklyInsights({ workouts: spikeW, goals: { sessions: 4 } }, '2026-07-13', '2026-07-17'); if (!(si.some(i => i.text.includes('Charge en pic')) && !si.some(i => i.text.includes('montes en volume')))) return false; const kmS = weeklyInsights({ workouts: [{ date: '2026-07-08', type: 'run', duration: 12, distance: 1.5, effort: 2 }], goals: { distance: 1 } }, '2026-07-06', '2026-07-11'); return kmS.some(i => i.text === '1.5 km couru — objectif 1 km atteint.') && !kmS.some(i => i.text.includes('km courus')); })(),
        ...
        sleepCoachDefersToPlan: typeof sleepCoachInsight === 'function' && typeof renderWeeklySleep === 'function' && !!document.getElementById('sleepCoach') && !!document.getElementById('sleepPlan') && (() => {
          // Quand un plan de recalage est ACTIF, il porte déjà l'action de coucher (cible chiffrée,
          // adaptative) juste sous le bilan. Le bilan ne doit alors PAS répéter une consigne de coucher
          // générique (« stabilise une heure fixe » / « vise un coucher 30 min plus tôt »…) qui ferait
          // doublon — et concurrencerait, plus vague, la cible précise du plan (§3/§4 ter, lecture cumulée).
          const pad = n => (n < 10 ? '0' + n : '' + n);
          const iso = off => { const d = new Date(localDate() + 'T12:00:00'); d.setDate(d.getDate() - off); return d.getFullYear() + '-' + pad(d.getMonth() + 1) + '-' + pad(d.getDate()); };
          const rec = [];
          for (let i = 0; i < 7; i++) rec.push({ date: iso(i), sleep: 6, bedtime: i % 2 ? '06:00' : '03:00' });
          const generic = /vise un coucher|stabilise|compte autant/;
          const saved = { r: state.recovery, p: state.sleepPlan };
          state.recovery = rec;
          state.sleepPlan = { active: true, targetTime: '23:30', startTime: '06:00', startKey: iso(6), stepMin: 25, stepDays: 1, lastReward: '' };
          renderWeeklySleep();
          const withPlan = document.getElementById('sleepCoach').textContent;
          const planText = document.getElementById('sleepPlan').textContent;
          state.sleepPlan = { active: false, targetTime: '', startTime: '', startKey: '', stepMin: 25, stepDays: 1, lastReward: '' };
          renderWeeklySleep();
          const withoutPlan = document.getElementById('sleepCoach').textContent;
          state.recovery = saved.r; state.sleepPlan = saved.p; renderWeeklySleep();
          return /coucher irrégulier/.test(withPlan) && !generic.test(withPlan)
        ...
        guidedTarget: !!document.getElementById('guidedTarget') && !!document.getElementById('guidedProgressionHint') && typeof progressionSuggestion === 'function' && typeof guidedProgressionLines === 'function' && (() => { const s = progressionSuggestion([{ date: '2026-06-08', exercises: [{ name: 'DC', load: 40, reps: 9 }] }], 'DC', { minReps: 8, maxReps: 12, increment: 2.5 }); if (!(s && s.nextReps === 10 && s.nextLoad === 40)) return false; const w = { action: 'weight', nextReps: 8, nextLoad: 42.5, lastReps: 12, lastLoad: 40 }; const green = guidedProgressionLines(w, false, 'Progression proposée : +0,5 kg si toutes les séries restent propres.'); if (green.target.indexOf('monte la charge') < 0 || green.hint.indexOf('Feu vert') < 0 || green.hint.indexOf('0,5 kg') >= 0) return false; const caut = guidedProgressionLines(w, true, 'Récupération du jour basse : répète le format.'); return caut.target.indexOf('monte la charge') < 0 && caut.target.indexOf('consolide') >= 0 && caut.hint === 'Récupération du jour basse : répète le format.'; })(),
        guidedFragileLive: typeof renderGuidedWorkout === 'function' && !!document.getElementById('guidedTarget') && !!document.getElementById('guidedRecoveryNote') && (() => {
          // Reprise le soir d'une séance ouverte le matin (récup OK) : #guidedRecoveryNote se recalcule
          // en direct (fragile), mais #guidedProgressionHint/#guidedTarget lisaient current.cautious GELÉ
          // au snapshot → « allège la charge » ET « monte la charge 💪 » côte à côte. Le rendu doit
          // désormais arbitrer par la récup LIVE (même exercice figé non-cautious, seule la récup change).
          const saved = { gw: guidedWorkout, gi: guidedIndex, w: state.workouts, r: state.recovery };
          const ex = { name: 'Squats à vide', sets: 3, reps: 12, load: 40, cautious: false, progressionHint: 'Progression proposée : +0,5 kg si toutes les séries restent propres.', setLogs: [] };
          guidedWorkout = { title: 'Séance test', why: '', duration: 45, exercises: [ex] };
          guidedIndex = 0;
          ...
          const fragileOk = fTgt.indexOf('consolide') >= 0 && fTgt.indexOf('monte la charge') < 0 && fHint.indexOf('Feu vert') < 0 && fHint.indexOf('0,5 kg') < 0 && fHint.indexOf('Récupération') >= 0 && fNote.indexOf('fragile') >= 0;
          guidedWorkout = saved.gw; guidedIndex = saved.gi; state.workouts = saved.w; state.recovery = saved.r;
          return greenOk && fragileOk;
        })(),
        ...
        kmRamp: typeof weeklyKmRamp === 'function' && !!document.getElementById('trailRamp') && (() => { const rm = weeklyKmRamp([{ type: 'run', date: '2026-07-08', distance: 25 }, { type: 'run', date: '2026-07-01', distance: 20 }], '2026-07-10'); if (!(rm && rm.thisWeekKm === 25 && rm.lastWeekKm === 20 && rm.rampPct === 25 && rm.zone === 'build' && rm.onBase === true)) return false; const tiny = weeklyKmRamp([{ type: 'run', date: '2026-07-09', distance: 9 }, { type: 'run', date: '2026-07-01', distance: 4 }], '2026-07-10'); return tiny && tiny.zone === 'high' && tiny.rampPct === 125 && tiny.onBase === false && weeklyKmRamp([], '2026-07-10') === null; })(),
        runRampSoften: typeof renderRunWeekGoal === 'function' && typeof weeklyKmRamp === 'function' && !!document.getElementById('runWeekGoal') && (() => { const savedW = state.workouts, savedG = JSON.parse(JSON.stringify(state.goals || {})); const today = localDate(), p = v => String(v).padStart(2, '0'); const d = n => { const x = new Date(today + 'T12:00:00'); x.setDate(x.getDate() - n); return x.getFullYear() + '-' + p(x.getMonth() + 1) + '-' + p(x.getDate()); }; state.goals = Object.assign({}, savedG, { distance: 20 }); state.workouts = [{ type: 'run', date: d(1), distance: 9 }, { type: 'run', date: d(8), distance: 4 }]; renderRunWeekGoal(); const low = document.getElementById('runWeekGoal').innerHTML; state.workouts = [{ type: 'run', date: d(1), distance: 30 }, { type: 'run', date: d(8), distance: 20 }]; renderRunWeekGoal(); const high = document.getElementById('runWeekGoal').innerHTML; state.workouts = savedW; state.goals = savedG; renderRunWeekGoal(); const softened = /ta base/.test(low) && !/hausse forte/.test(low); const warned = /hausse forte/.test(high) && !/ta base/.test(high); return softened && warned; })(),
        ultraDownhill: typeof downhillPrep === 'function' && !!document.getElementById('ultraDownhill') && (() => { const base = downhillPrep(600, null, 0), spec = downhillPrep(500, 30, 42), hilly = downhillPrep(1500, 30, 80), race = downhillPrep(600, 8, 42), taperMarathon = downhillPrep(600, 13, 42); return downhillPrep(0, 30, 42) === null && base.window === 'base' && base.sessionsPerWeek === 1 && spec.window === 'specific' && spec.sessionsPerWeek === 1 && hilly.sessionsPerWeek === 2 && race.window === 'race' && race.sessionsPerWeek === 0 && taperMarathon.window === 'race' && !!(typeof taperPlan === 'function' && taperPlan(13, 42)); })(),
        ...
        coachWeight: typeof energyPlan === 'function' && typeof basalMetabolicRate === 'function' && !!document.getElementById('coachWeightBody') && !!document.getElementById('coachAge') && (() => { const p = energyPlan({ weight: 80, height: 180, age: 30, sex: 'homme', sessionsPerWeek: 4, targetWeight: 72, todayKey: '2026-07-12' }); return p && p.bmr === 1780 && p.tdee === 2759 && p.goal === 'perte' && p.weeks === 15 && p.targetDate === '2026-10-25' && p.proteinG === 192 && energyPlan({ weight: 80, height: 180, age: 30, targetWeight: 0 }) === null; })(),
        ...
        strengthStd: typeof strengthStandards === 'function' && !!document.getElementById('strengthStandards') && (() => {
          // poids du corps (reps, sans poids requis) + gilet lesté (reps équivalentes) ; PAS de barre ; sans regex
          const bwx = strengthStandards([{ date: '2026-07-01', type: 'strength', exercises: [{ name: 'Tractions', sets: 4, reps: 12, load: 0 }] }], 0);
          const vest = strengthStandards([{ date: '2026-07-01', type: 'strength', exercises: [{ name: 'Tractions', setLogs: [{ reps: 5, load: 20 }] }] }], 80);
          const noBar = strengthStandards([{ date: '2026-07-01', type: 'strength', exercises: [{ name: 'Squat', reps: 5, load: 82, sets: 5 }] }], 80);
          return Array.isArray(bwx) && bwx.length === 1 && bwx[0].key === 'pullup' && bwx[0].effReps === 12 && bwx[0].level === 'Intermédiaire'
        ...
        vestProg: typeof vestProgression === 'function' && !!document.getElementById('vestProgression') && (() => {
          // double progression gilet : série lestée -> conseil ; poids du corps ignoré ; garde-fous ; sans regex
          const r = vestProgression([{ date: '2026-07-01', type: 'strength', exercises: [{ name: 'Tractions', setLogs: [{ reps: 8, load: 15 }] }] }]);
          return Array.isArray(r) && r.length === 1 && r[0].key === 'pullup' && r[0].action === 'addReps' && r[0].load === 15
            && vestProgression([{ date: '2026-07-01', type: 'strength', exercises: [{ name: 'Tractions', reps: 12, load: 0, sets: 4 }] }]).length === 0
            && vestProgression(null).length === 0;
        })(),
        ...
        coachMeasure: typeof recompositionInsight === 'function' && recompositionInsight(-0.3, -2).key === 'recomp' && recompositionInsight(-2, -2).key === 'fatloss' && recompositionInsight(-0.2, -0.2) === null && recompositionInsight(2, 2, 'perte').message.indexOf('prise de muscle') === -1 && recompositionInsight(2, 2, 'perte').message.indexOf('vises la perte') !== -1 && recompositionInsight(-2, -2, 'prise').message.indexOf('bien engagée') === -1 && recompositionInsight(2, 2, 'prise').message.indexOf('prise de muscle') !== -1,
        ...
        targetAdvice: typeof weightTargetAdvice === 'function' && !!document.getElementById('coachTargetAdvice') && (() => { const base = { weight: 81, height: 174, age: 30, sex: 'homme' }; const ok = weightTargetAdvice({ ...base, targetWeight: 75, fitnessObjective: 'athletique' }); if (!ok || ok.level !== 'ok' || ok.direction !== 'perte') return false; const bas = weightTargetAdvice({ ...base, targetWeight: 55, fitnessObjective: 'athletique' }); if (bas.level !== 'stop' || bas.targetBmi >= 18.5) return false; const contra = weightTargetAdvice({ ...base, targetWeight: 70, fitnessObjective: 'muscle' }); if (contra.level !== 'stop') return false; const maint = weightTargetAdvice({ ...base, targetWeight: 81, fitnessObjective: 'athletique' }); if (maint.direction !== 'maintien' || weightTargetAdvice({ weight: 81, height: 174 }) !== null) return false; const prof = { weight: 92, height: 178, age: 30, sex: 'homme', activityLevel: 'modere', sessionsPerWeek: 4, targetWeight: 78 }; const plan = typeof energyPlan === 'function' ? energyPlan({ ...prof, todayKey: '2026-07-20' }) : null; const adv = weightTargetAdvice(prof); return !!plan && plan.ratePerWeek === adv.ratePerWeek && plan.weeks === adv.weeks; })(),
        ...
        zfRestFallback: typeof renderAthlete === 'function' && typeof zoneFreshness === 'function' && !!document.getElementById('zoneFreshness') && (() => { const savedW = state.workouts, today = localDate(), p = v => String(v).padStart(2, '0'); const d = n => { const x = new Date(today + 'T12:00:00'); x.setDate(x.getDate() - n); return x.getFullYear() + '-' + p(x.getMonth() + 1) + '-' + p(x.getDate()); }; state.workouts = [{ type: 'sport', date: d(1), duration: 30, effort: 5, xp: 10, exercises: [{ name: 'Gainage planche', sets: 3 }] }]; renderAthlete(); const html = document.getElementById('zoneFreshness').innerHTML; state.workouts = savedW; renderAthlete(); return /Prêt aujourd/.test(html) && !/journée légère/.test(html); })(),
        weekLoadNote: typeof renderAthlete === 'function' && typeof weekLoadNote === 'function' && !!document.getElementById('weekLoadAdvice') && (() => { const savedW = state.workouts, savedR = state.recovery, today = localDate(), p = v => String(v).padStart(2, '0'); const d = n => { const x = new Date(today + 'T12:00:00'); x.setDate(x.getDate() - n); return x.getFullYear() + '-' + p(x.getMonth() + 1) + '-' + p(x.getDate()); }; state.workouts = [{ type: 'sport', date: d(2), duration: 30, effort: 2, xp: 10 }, { type: 'sport', date: d(10), duration: 90, effort: 3, xp: 10 }, { type: 'sport', date: d(14), duration: 90, effort: 3, xp: 10 }, { type: 'sport', date: d(20), duration: 90, effort: 3, xp: 10 }]; state.recovery = [{ date: today, sleep: 5, fatigue: 2, soreness: 2 }]; renderAthlete(); const txt = document.getElementById('weekLoadAdvice').textContent; state.workouts = savedW; state.recovery = savedR; renderAthlete(); return /garde la prochaine séance facile/.test(txt) && /charge en baisse/.test(txt) && !/tu peux remonter/.test(txt); })(),
        ...
        weekProgramTaper: typeof buildTrainingWeek === 'function' && typeof taperPlan === 'function' && (() => {
          const kmOf = p => (p && p.days ? p.days : []).reduce((s, d) => s + (Number(d.km) || 0) + (d.runs || []).reduce((a, r) => a + (Number(r.km) || 0), 0), 0);
          const full = buildTrainingWeek(['legs'], 2, 3, false, { weeklyKm: 40, week: 5 });
          const tap = buildTrainingWeek(['legs'], 2, 3, false, { weeklyKm: 40, week: 5, raceDaysLeft: 4, raceKm: 42 });
          const far = buildTrainingWeek(['legs'], 2, 3, false, { weeklyKm: 40, week: 5, raceDaysLeft: 60, raceKm: 42 });
          if (!full || !tap || !far || full.taper || far.taper || !tap.taper) return false;
        ...
        coachDietBreakNoTighten: !!document.getElementById('coachWeightBody') && typeof renderCoachWeight === 'function' && (() => {
          // Contradiction inter-surfaces sur la carte « Coach Poids » : quand la PAUSE DIÈTE est due
          // (déficit long + perte réelle), le rythme récent est lent PAR DÉFINITION (le plateau est CE
          // qui déclenche la pause) → paceStatus renvoie 'slow' et la realLine disait « resserre un peu
          // le déficit » juste au-dessous d'un « remonte à ta maintenance » (+~700 kcal/j). Deux conseils
          // nutritionnels opposés. On rend la carte dans ce scénario exact et on vérifie la cohérence.
          ...
          // k=0 = pesée la plus récente (il y a 7 j, 79,0 kg) … k=12 (il y a 91 j, 84,4 kg). Les 6 plus
          // récentes quasi plates (79,0→79,5) → rythme récent −0,10 kg/sem → paceStatus 'slow' ; la série
          // de 12 sem. avec 5,4 kg perdus → dietBreak.due. Ordre CHRONOLOGIQUE (le rendu lit .at(-1)).
          const val = k => k <= 5 ? Math.round((79.0 + k * 0.1) * 10) / 10 : Math.round((79.5 + (k - 5) * 0.7) * 10) / 10;
          state.weights = Array.from({ length: 13 }, (_, i) => { const k = 12 - i; return { date: dk(7 + 7 * k), value: val(k) }; });
          state.goals = { ...state.goals, targetWeight: 75 };
          state.profile = { ...state.profile, weight: 79, height: 180, age: 30, sex: 'homme', activityLevel: '' };
          renderCoachWeight();
          const html = document.getElementById('coachWeightBody').innerHTML;
          const pauseShown = /PAUSE DIÈTE/.test(html);         // la pause est bien recommandée
          const slowShown = /rythme réel/.test(html);          // la realLine 'slow' est bien rendue
          const noTighten = !/resserre un peu le déficit/.test(html); // …sans conseiller de resserrer
        ...
        a11yObjective: (() => { const sel = document.getElementById('objectiveSelect'), live = ['objectiveResult', 'runPlanResult', 'quickSessionResult', 'wpResult'].map(id => document.getElementById(id)); return !!sel && (sel.getAttribute('aria-label') || '').length > 3 && live.every(el => !!el && el.getAttribute('aria-live') === 'polite'); })(),
        ...
        whatsNew: typeof whatsNewSince === 'function' && typeof compareVersions === 'function' && typeof CHANGELOG !== 'undefined' && !!document.getElementById('whatsNewCard') && (() => { const log = [{ v: '1.9.190', emoji: '✨', text: 'C' }, { v: '1.9.189', emoji: '📈', text: 'B' }, { v: '1.9.188', emoji: '🧘', text: 'A' }]; const seen = whatsNewSince('1.9.188', log); return compareVersions('1.10.0', '1.9.99') === 1 && whatsNewSince('', log).length === 0 && seen.length === 2 && seen[0].v === '1.9.190' && whatsNewSince('1.9.190', log).length === 0 && Array.isArray(CHANGELOG) && String(CHANGELOG[0].v || '').split('.').length === 3; })(),
        ...
        coachFocus: typeof adaptiveCoachFocus === 'function' && typeof renderCoachFocus === 'function' && !!document.getElementById('coachFocusPanel') && !!document.getElementById('coachFocus') && (() => {
          ...
          // Anti-redondance (curation §3) : la carte porte sa propre action de coucher juste sous cet
          // insight → l'insight (verdict urgent) garde le DIAGNOSTIC mais ne répète plus la consigne
          // « heure fixe », que l'action, elle, porte bien.
          if (/heure (de coucher )?fixe/.test(fSleep.insight)) return false;
          if (!/heure fixe ce soir/.test(fSleep.action)) return false;
          // Cohérence insight↔action : sommeil SOLIDE (tone 'ok') → action de MAINTIEN, jamais la correction « coucher 30 min plus tôt ».
          ...
          // #689 : en ton revive (sport dormant depuis le 06-20 → headline « Reprends l'entraînement »),
          // sessionGoalAhead ne colle plus la clause de dispense « rien ne t'oblige à t'entraîner
          // aujourd'hui » sous « relance dès aujourd'hui » (contradiction retirée) ; l'invitation
          // « engranger une séance d'avance » subsiste, cohérente dans tous les tons.
          const fSessRevive = adaptiveCoachFocus({ workouts: [{ date: '2026-06-20' }], goals: { sessions: 2 }, recovery: [{ date: '2026-07-13', sleep: 8, fatigue: 1, soreness: 1 }] }, '2026-07-13');
          if (!(fSessRevive.tone === 'revive' && fSessRevive.sessionGoalAhead === 100 && /engranger une séance d.avance/.test(fSessRevive.insight) && !/rien ne t.oblige à t.entraîner/.test(fSessRevive.insight))) return false;
          ...
          // Dimanche 07-19 (dernier jour), 90 min pour 1 seul jour → serré.
          const fFocusTight = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-05', minutes: 30 }, { date: '2026-07-13', minutes: 30 }] }, '2026-07-19');
          if (!(fFocusTight.focusGoalPace === 'tight' && /Serré : 90 min restantes pour 1 jour/.test(fFocusTight.insight))) return false;
          // Sans check-in de récup du jour → aucun alignement, focusGoalFresh null.
          if (fFocusTight.focusGoalFresh !== null) return false;
          ...
          // Tout au top (8/1/1) → trois forces à égalité → focusFreshDriver null, pas de moteur nommé.
          if (fFocusFresh.focusFreshDriver !== null || /nourrit cette fra[îi]cheur mentale/.test(fFocusFresh.insight)) return false;
          ...
          // #678 non-redite : focusGoalDrained porte déjà l'action → le driver ne ré-sert plus « acharnement » (un seul /acharn/, celui du parent « t'acharner »).
          if (/pas l.acharnement du jour/.test(fDrainSleep.insight) || (fDrainSleep.insight.match(/acharn/g) || []).length !== 1) return false;
          ...
          // PRENDRE DE L'AVANCE (focusGoalAhead, #535) — pendant PROACTIF de focusGoalFresh : objectif focus LARGE (onpace, mardi 07-14) + readiness au vert le jour même → invitation à engranger un coussin. Session focus du 07-14 pour garder le pilier focus malgré le check-in. sleep 8/fat 1/sore 1 → 100.
          const fFocusAhead = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-05', minutes: 30 }, { date: '2026-07-13', minutes: 30 }, { date: '2026-07-14', minutes: 10 }], recovery: [{ date: '2026-07-14', sleep: 8, fatigue: 1, soreness: 1 }] }, '2026-07-14');
          if (!(fFocusAhead.pillar === 'focus' && fFocusAhead.focusGoalPace === 'onpace' && fFocusAhead.focusGoalAhead === 100 && /ta tête est claire ce matin \\(readiness 100\\/100\\)/.test(fFocusAhead.insight) && /prendre de l.avance sur l.objectif tant que c.est facile/.test(fFocusAhead.insight))) return false;
          // Mutuellement exclusif : branche serrée → focusGoalAhead null ; objectif large × readiness moyenne (6/3/3 → 60) → aucune invitation.
          ...
          // CURATION §4 ter (#672) : la note qui NOMME le moteur ne RÉPÈTE plus le « engranger/banker un bloc » déjà posé par focusGoalAhead deux phrases plus haut.
          if (/engranger un bloc|banker un bloc/.test(fAheadDrv.insight)) return false;
          // CERVEAU À PLAT × MARGE (focusMarginDrained) — trou symétrique de focusGoalAhead dans la branche onpace : objectif focus LARGE (onpace) × readiness à plat le jour même (5/4/4 → 40) → on rassure (lève le pied, la marge encaisse), là où avant le coach restait muet. sleep 5/fat 4/sore 4 → 40.
          const fMarginDrained = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-05', minutes: 30 }, { date: '2026-07-13', minutes: 30 }, { date: '2026-07-14', minutes: 10 }], recovery: [{ date: '2026-07-14', sleep: 5, fatigue: 4, soreness: 4 }] }, '2026-07-14');
          if (!(fMarginDrained.pillar === 'focus' && fMarginDrained.focusGoalPace === 'onpace' && fMarginDrained.focusMarginDrained === 40 && fMarginDrained.focusGoalAhead === null && /ton énergie mentale est basse ce matin \\(readiness 40\\/100\\)/.test(fMarginDrained.insight) && /ta marge encaisse ce jour au ralenti/.test(fMarginDrained.insight))) return false;
          // Mutuellement exclusif : au vert (fFocusAhead → focusGoalAhead) et branche serrée à plat → focusMarginDrained null.
          if (fFocusAhead.focusMarginDrained !== null || fAheadMid.focusMarginDrained !== null) return false;
          ...
          // GARDE-FOU DE TON (sessionGoalBonus) : objectif hebdo CALENDAIRE tenu (2/2, lun+mar) MAIS momentum glissant en recul (prev 3 j : 06/08/10 → rebuild) × readiness au vert → le ton est « rebuild » (« s'essouffle… un petit geste suffit à repartir »). Le bonus « aucune obligation de t'y remettre » le contredirait dans le MÊME insight → il doit être MUET ; le « Objectif hebdo déjà tenu 💪 » (constat) reste.
          const fSessBonusRebuild = adaptiveCoachFocus({ workouts: [{ date: '2026-07-06' }, { date: '2026-07-08' }, { date: '2026-07-10' }, { date: '2026-07-13' }, { date: '2026-07-14' }], goals: { sessions: 2 }, recovery: [{ date: '2026-07-19', sleep: 8, fatigue: 1, soreness: 1 }] }, '2026-07-19');
          if (!(fSessBonusRebuild.pillar === 'sport' && fSessBonusRebuild.tone === 'rebuild' && fSessBonusRebuild.sessionGoalBonus === null && /Un petit geste suffit à repartir/.test(fSessBonusRebuild.insight) && /Objectif hebdo déjà tenu : 2\\/2 séances/.test(fSessBonusRebuild.insight) && !/aucune obligation/.test(fSessBonusRebuild.insight) && !/du gain offert/.test(fSessBonusRebuild.insight))) return false;
          ...
          if (fPureRun.pushPullGuard !== null || fPureRun.sportNeglectGuard !== null) return false;
          if (fPureRun.runVolumeGuard !== null) return false; // runs sans distance → weeklyKmRamp nul → pas de note
          // Montée de KILOMÉTRAGE trop rapide (runVolumeGuard) : 20 km la semaine passée → 30 km cette semaine (+50 %, zone 'high') sur une vraie base → note « surveille ta montée de kilométrage ».
          const fRamp = adaptiveCoachFocus({ workouts: [{ date: '2026-07-05', type: 'run', distance: 20 }, { date: '2026-07-12', type: 'run', distance: 30 }] }, '2026-07-16');
          if (!(fRamp.pillar === 'sport' && fRamp.runVolumeGuard && fRamp.runVolumeGuard.rampPct === 50 && fRamp.runVolumeGuard.thisWeekKm === 30 && /surveille ta montée de kilométrage/.test(fRamp.insight))) return false;
          // Zone CHRONIQUEMENT délaissée sur 4 semaines (sportNeglectGuard) : muscu équilibrée haut du corps + abdos/fessiers, ZÉRO jambe sur le mois → note « ta zone la plus délaissée, c'est les jambes ».
          const fNeglect = adaptiveCoachFocus({ workouts: [{ date: '2026-06-22', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-06-29', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-07-06', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-06-24', exercises: [{ name: 'Tractions', sets: 5, reps: 8 }] }, { date: '2026-07-01', exercises: [{ name: 'Tractions', sets: 5, reps: 8 }] }, { date: '2026-07-08', exercises: [{ name: 'Tractions', sets: 5, reps: 8 }] }, { date: '2026-06-26', exercises: [{ name: 'Gainage planche', sets: 4 }] }, { date: '2026-07-10', exercises: [{ name: 'Gainage planche', sets: 4 }] }, { date: '2026-06-28', exercises: [{ name: 'Pont fessier', sets: 4 }] }, { date: '2026-07-04', exercises: [{ name: 'Pont fessier', sets: 4 }] }] }, '2026-07-16');
          if (!(fNeglect.pillar === 'sport' && fNeglect.sportNeglectGuard && fNeglect.sportNeglectGuard.zone === 'legs' && fNeglect.sportNeglectGuard.sets === 0 && /ta zone la plus délaissée/.test(fNeglect.insight) && /les jambes/.test(fNeglect.insight))) return false;
          // Balance POUSSÉE ↔ TIRAGE (pushPullGuard) : muscu du mois très push-heavy (35 séries de pompes, 3 de tractions) → note « coiffe des rotateurs », côté dos à recharger.
          const fPush = adaptiveCoachFocus({ workouts: [{ date: '2026-06-22', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-06-25', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-06-29', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-07-03', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-07-07', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-07-11', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-07-14', exercises: [{ name: 'Pompes classiques', sets: 5, reps: 12 }] }, { date: '2026-06-24', exercises: [{ name: 'Tractions', sets: 3, reps: 8 }] }] }, '2026-07-16');
          if (!(fPush.pillar === 'sport' && fPush.pushPullGuard && fPush.pushPullGuard.zone === 'push-heavy' && fPush.pushPullGuard.push === 35 && fPush.pushPullGuard.pull === 3 && /coiffe des rotateurs/.test(fPush.insight) && /Ajoute du dos/.test(fPush.insight))) return false;
          ...
          // Fruits & légumes délaissés (fruitGuard) : nutrition suivie 10 j via protéines (sans profil → pas de pente d'intrant), ZÉRO fruit coché → note micronutriments.
          const fFruitNut = ['2026-07-05', '2026-07-07', '2026-07-09', '2026-07-10', '2026-07-11', '2026-07-12', '2026-07-13', '2026-07-14', '2026-07-15', '2026-07-16'].map(d => ({ date: d, protein: 100 }));
          const fFruit = adaptiveCoachFocus({ nutrition: fFruitNut }, '2026-07-16');
          ...
          // Coach INTER-PILIER (pendant PRISE) : sommeil COURT × objectif de PRISE → frein invisible musculaire (sleepGainGuard).
          const fSleepGain = adaptiveCoachFocus({ nutrition: fWeightNut, goals: { targetWeight: 90 }, weights: [{ date: '2026-06-01', value: 82 }, { date: '2026-07-14', value: 85 }], recovery: fShortSleep }, '2026-07-16');
          if (!(fSleepGain.pillar === 'nutrition' && fSleepGain.sleepGainGuard === 6 && fSleepGain.sleepFatLossGuard === null && /Et surveille un frein invisible : tu dors 6 h en moyenne ces derniers jours \\(dette de 21 h sur 14 j\\), sous les 7 h/.test(fSleepGain.insight) && /transformer tes calories en muscle/.test(fSleepGain.insight))) return false;
          // Prise × sommeil solide (8 h) → aucun frein invisible.
          const fGainRested = adaptiveCoachFocus({ nutrition: fWeightNut, goals: { targetWeight: 90 }, weights: [{ date: '2026-06-01', value: 82 }, { date: '2026-07-14', value: 85 }], recovery: fShortSleep.map(r => ({ date: r.date, sleep: 8 })) }, '2026-07-16');
          if (fGainRested.sleepGainGuard !== null || /frein invisible/.test(fGainRested.insight)) return false;
          // Coach INTER-PILIER (pendant SPORT) : sommeil COURT (avg < 7, non urgent) × pilier SPORT → socle invisible des gains d'entraînement (sleepTrainGuard).
          const fSportWk = [{ date: '2026-07-04' }, { date: '2026-07-06' }, { date: '2026-07-08' }, { date: '2026-07-15' }];
          const fSleepTrain = adaptiveCoachFocus({ workouts: fSportWk, recovery: fShortSleep }, '2026-07-16');
          if (!(fSleepTrain.pillar === 'sport' && fSleepTrain.sleepTrainGuard === 6 && fSleepTrain.sleepFatLossGuard === null && /Et n.oublie pas le socle invisible de tes gains : tu dors 6 h en moyenne ces derniers jours \\(dette de 21 h sur 14 j\\), sous les 7 h/.test(fSleepTrain.insight) && /Bien dormir d[ée]multiplie l.effort/.test(fSleepTrain.insight))) return false;
          ...
          if (!(fSleepFocus.pillar === 'focus' && fSleepFocus.sleepFocusGuard === 6 && fSleepFocus.sleepTrainGuard === null && /Et n.oublie pas ce qui alimente ta concentration : tu dors 6 h en moyenne ces derniers jours \\(dette de 21 h sur 14 j\\), sous les 7 h/.test(fSleepFocus.insight) && /d[ée]multiplie chaque bloc de focus/.test(fSleepFocus.insight))) return false;
          // Focus × sommeil solide (8 h) → aucun carburant caché.
          const fFocusRested = adaptiveCoachFocus({ focusSessions: fFocusDecl, recovery: fShortSleep.map(r => ({ date: r.date, sleep: 8 })) }, '2026-07-16');
          if (fFocusRested.sleepFocusGuard !== null || /alimente ta concentration/.test(fFocusRested.insight)) return false;
          // Coach INTER-PILIER (pendant FOCUS) : durée OK (8 h) mais COUCHER irrégulier (±150 min) × pilier FOCUS → note « horloge stable » (bedtimeFocusGuard), distincte de la note durée.
          const fIrregBed = [];
          const fbd = ['03','04','05','06','07','08','09','10','11','12','13','14','15','16'];
          fbd.forEach((d, i) => fIrregBed.push({ date: '2026-07-' + d, sleep: 8, bedtime: i % 2 ? '03:00' : '22:00' }));
          const fWobbly = adaptiveCoachFocus({ focusSessions: fFocusDecl, recovery: fIrregBed }, '2026-07-16');
          if (!(fWobbly.pillar === 'focus' && fWobbly.bedtimeFocusGuard === 150 && fWobbly.sleepFocusGuard === null && /Ta dur[ée]e de sommeil tient, mais tes couchers partent dans tous les sens \\(±150 min d.un soir [àa] l.autre\\)/.test(fWobbly.insight) && /Se coucher [àa] heure fixe compte ici autant que le nombre d.heures/.test(fWobbly.insight))) return false;
          if (/alimente ta concentration/.test(fWobbly.insight)) return false;
          ...
          // HYDRATATION basse × pilier FOCUS (durée OK, pas de coucher saisi → 3 notes sommeil muettes) → levier aigu de la concentration (hydrationFocusGuard).
          const fPlainSleep = ['10','11','12','13','14','15','16'].map(d => ({ date: '2026-07-' + d, sleep: 8 }));
          const fDryNut = ['11','12','14','15'].map(d => ({ date: '2026-07-' + d, water: 4 }));
          const fDry = adaptiveCoachFocus({ focusSessions: fFocusDecl, recovery: fPlainSleep, nutrition: fDryNut }, '2026-07-16');
          if (!(fDry.pillar === 'focus' && fDry.hydrationFocusGuard === 4 && fDry.sleepFocusGuard === null && fDry.bedtimeFocusGuard === null && fDry.bedtimeFocusTrend === null && /Et un levier imm[ée]diat, souvent n[ée]glig[ée] : tu bois 4 verres d.eau par jour ces derniers jours, sous les 8/.test(fDry.insight) && /Contrairement au sommeil, [çc]a se corrige en minutes/.test(fDry.insight))) return false;
          if (/alimente ta concentration/.test(fDry.insight)) return false;
          ...
          // Coach × PROTÉINE (proteinTrainGuard) : sport en décrochage MAIS actif (prev 4 j, recent 2 j) → pilier SPORT choisi malgré la nutrition, sommeil/hydratation/mobilité muets, protéines chroniquement basses (0/4 à la cible 135 g) → note « matériau des gains ».
          const fProtWk = ['2026-07-03', '2026-07-05', '2026-07-07', '2026-07-09', '2026-07-11', '2026-07-15'].map(d => ({ date: d }));
          const fProtNut = [{ date: '2026-07-11', protein: 90 }, { date: '2026-07-12', protein: 90 }, { date: '2026-07-14', protein: 90 }, { date: '2026-07-15', protein: 90 }];
          const fProtLow = adaptiveCoachFocus({ workouts: fProtWk, profile: { weight: 75, goal: 'muscle' }, nutrition: fProtNut }, '2026-07-16');
          if (!(fProtLow.pillar === 'sport' && fProtLow.proteinTrainGuard === 0 && fProtLow.sleepTrainGuard === null && fProtLow.hydrationTrainGuard === null && fProtLow.mobilityTrainGuard === null && /Et pense au mat[ée]riau de tes gains : sur tes 4 derniers jours renseign[ée]s, tu n.atteins ta cible prot[ée]ines \\(135 g\\) que 0\\/4/.test(fProtLow.insight) && /l.entra[îi]nement ne fait que casser le muscle/.test(fProtLow.insight))) return false;
          if (/socle invisible/.test(fProtLow.insight) || /carburant qu.on oublie/.test(fProtLow.insight) || /c[ôo]t[ée] r[ée]cup[ée]ration/.test(fProtLow.insight)) return false;
          ...
          // Coach de la PROGRESSION DE FORCE (sportPlateau) : Squat chargé régulier dont le 1RM estimé ne progresse plus → plateau nommé + geste concret dans l'insight.
          const plWo = (d, ld, rp) => ({ date: d, exercises: [{ name: 'Squat', setLogs: [{ completed: true, load: ld, reps: rp }] }] });
          const fPlat = adaptiveCoachFocus({ workouts: [plWo('2026-07-01', 100, 5), plWo('2026-07-04', 98, 5), plWo('2026-07-08', 99, 5), plWo('2026-07-11', 100, 5), plWo('2026-07-14', 98, 5)] }, '2026-07-16');
          if (!(fPlat.pillar === 'sport' && fPlat.sportPlateau && fPlat.sportPlateau.exercise === 'Squat' && /ton Squat marque le pas/.test(fPlat.insight) && /1RM estim[ée] stagne autour de [\\d,]+ kg depuis 3 séances/.test(fPlat.insight))) return false;
          // Exclusion mutuelle : quand un plateau parle, la note de progression se tait (jamais « stagne » et « grimpe » ensemble).
          if (fPlat.sportProgress !== null || /gagne du terrain/.test(fPlat.insight)) return false;
          // Coach de la PROGRESSION DE FORCE (sportProgress) : Squat chargé qui grimpe (90→110) → lift nommé + projection du prochain palier dans l'insight, aucun plateau.
          const fPlatUp = adaptiveCoachFocus({ workouts: [plWo('2026-07-01', 90, 5), plWo('2026-07-04', 95, 5), plWo('2026-07-08', 100, 5), plWo('2026-07-11', 105, 5), plWo('2026-07-14', 110, 5)] }, '2026-07-16');
          if (fPlatUp.sportPlateau !== null || /marque le pas/.test(fPlatUp.insight)) return false;
          if (!(fPlatUp.sportProgress && fPlatUp.sportProgress.exercise === 'Squat' && fPlatUp.sportProgress.milestone === 130 && fPlatUp.sportProgress.weeks === 1 && /ton Squat gagne du terrain — 1RM estimé à 128,5 kg \\(\\+12,65 kg\\/sem\\)/.test(fPlatUp.insight) && /tu passes la barre des 130 kg dans ~1 semaine/.test(fPlatUp.insight))) return false;
          // Coach du RECORD DU JOUR (sportRecordToday) : Squat déjà au palmarès (meilleur passé 104×5) puis 110×5 AUJOURD'HUI → PR battu, nommé et chiffré dans l'insight ; séance déjà faite.
          const fRec = adaptiveCoachFocus({ workouts: [plWo('2026-07-08', 100, 5), plWo('2026-07-11', 102, 5), plWo('2026-07-14', 104, 5), plWo('2026-07-16', 110, 5)] }, '2026-07-16');
          if (!(fRec.pillar === 'sport' && fRec.doneToday === true && fRec.sportRecordToday && fRec.sportRecordToday.exercise === 'Squat' && fRec.sportRecordToday.e1rm === 128.5 && /tu viens de battre ton record sur le Squat/.test(fRec.insight) && /1RM estimé à 128,5 kg/.test(fRec.insight))) return false;
          // Premier passage sur un exercice aujourd'hui (jamais pratiqué avant) → pas un « record » trivial.
          const fRecFirst = adaptiveCoachFocus({ workouts: [plWo('2026-07-08', 100, 5), plWo('2026-07-11', 102, 5), plWo('2026-07-14', 104, 5), { date: '2026-07-16', exercises: [{ name: 'Soulevé de terre', setLogs: [{ completed: true, load: 140, reps: 3 }] }] }] }, '2026-07-16');
          if (fRecFirst.sportRecordToday !== null || /battre ton record/.test(fRecFirst.insight)) return false;
          // Coach du RECORD DE RÉPÉTITIONS au poids du corps (sportRepRecordToday) : Tractions sans charge, meilleur passé 10 reps puis 13 AUJOURD'HUI → PR de reps nommé, sportRecordToday muet (aucune charge).
          const bwWo = (date, reps) => ({ date, exercises: [{ name: 'Tractions', setLogs: [{ completed: true, load: 0, reps }] }] });
          const fRep = adaptiveCoachFocus({ workouts: [bwWo('2026-07-08', 8), bwWo('2026-07-11', 9), bwWo('2026-07-14', 10), bwWo('2026-07-16', 13)] }, '2026-07-16');
          if (!(fRep.pillar === 'sport' && fRep.doneToday === true && fRep.sportRecordToday === null && fRep.sportRepRecordToday && fRep.sportRepRecordToday.exercise === 'Tractions' && fRep.sportRepRecordToday.reps === 13 && fRep.sportRepRecordToday.prev === 10 && /tu viens de battre ton record de r[ée]p[ée]titions sur le Tractions/.test(fRep.insight) && /13 reps au poids du corps \\(ton meilleur pass[ée] : 10\\)/.test(fRep.insight))) return false;
          // Égaler le meilleur passé au poids du corps → pas un record (mute).
          const fRepTie = adaptiveCoachFocus({ workouts: [bwWo('2026-07-08', 8), bwWo('2026-07-11', 9), bwWo('2026-07-14', 12), bwWo('2026-07-16', 12)] }, '2026-07-16');
          if (fRepTie.sportRepRecordToday !== null || /record de r[ée]p[ée]titions/.test(fRepTie.insight)) return false;
          ...
          // Régularité qui grimpe (semaine récente 4 > précédente 1).
          const fProtUp = adaptiveCoachFocus({ profile: { weight: 80, goal: 'force' }, nutrition: [{ date: '2026-07-05', protein: 160 }, { date: '2026-07-10', protein: 160 }, { date: '2026-07-11', protein: 160 }, { date: '2026-07-13', protein: 160 }, { date: '2026-07-14', protein: 160 }, { date: '2026-07-15', protein: 50 }] }, '2026-07-16');
          if (!(fProtUp.proteinTrend === 3 && /r[ée]gularit[ée] grimpe : 4 jours à la cible cette semaine vs 1 la précédente \\(\\+3\\)/.test(fProtUp.insight))) return false;
          // Sans semaine précédente renseignée → proteinTrend null, aucune note de pente.
          if (adaptiveCoachFocus({ profile: { weight: 80, goal: 'force' }, nutrition: [{ date: '2026-07-10', protein: 160 }, { date: '2026-07-12', protein: 160 }, { date: '2026-07-15', protein: 50 }] }, '2026-07-16').proteinTrend !== null) return false;
          // PENTE d'HYDRATATION (relais quand les protéines sont muettes) : sans profil, l'eau grimpe (récente 6 j à 8 verres vs précédente 3) → note « côté hydratation, ça suit ».
          const fHydUp = adaptiveCoachFocus({ nutrition: [{ date: '2026-07-03', water: 8 }, { date: '2026-07-04', water: 8 }, { date: '2026-07-05', water: 8 }, { date: '2026-07-10', water: 8 }, { date: '2026-07-11', water: 8 }, { date: '2026-07-12', water: 8 }, { date: '2026-07-13', water: 8 }, { date: '2026-07-14', water: 8 }, { date: '2026-07-15', water: 8 }] }, '2026-07-16');
          if (!(fHydUp.pillar === 'nutrition' && fHydUp.hydrationTrend === 3 && fHydUp.proteinTrend === null && /côté hydratation, ça suit : 6 jours à tes 8 verres cette semaine vs 3 la précédente \\(\\+3\\)/.test(fHydUp.insight))) return false;
          // Quand la pente PROTÉINES parle, l'hydratation se tait (un seul intrant à la fois) même si l'eau décroche en parallèle.
          const fHydMute = adaptiveCoachFocus({ profile: { weight: 80, goal: 'force' }, nutrition: [{ date: '2026-07-05', protein: 160 }, { date: '2026-07-10', protein: 160 }, { date: '2026-07-11', protein: 160 }, { date: '2026-07-13', protein: 160 }, { date: '2026-07-14', protein: 160 }, { date: '2026-07-03', water: 8 }, { date: '2026-07-04', water: 8 }, { date: '2026-07-06', water: 8 }, { date: '2026-07-07', water: 8 }, { date: '2026-07-08', water: 8 }, { date: '2026-07-09', water: 8 }, { date: '2026-07-15', water: 3 }] }, '2026-07-16');
          if (!(fHydMute.proteinTrend === 3 && fHydMute.hydrationTrend === null && !/hydratation/.test(fHydMute.insight))) return false;
          ...
          // FREIN FOCUS (pendant focus de #585) : tête à plat (readiness < 50) → l'insight dit « focus court, soigne ta récup », donc l'action NE pousse PLUS un bloc habituel ni un créneau ; elle retombe au bloc court de base.
          const fDrainCut = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-07', minutes: 30, task: 'Compta' }, { date: '2026-07-08', minutes: 30, task: 'Compta' }, { date: '2026-07-09', minutes: 30, task: 'Compta' }, { date: '2026-07-13', minutes: 30, task: 'Compta' }], agenda: [{ id: 'a', date: '2026-07-19', time: '09:00', durationMin: 60 }], recovery: [{ date: '2026-07-19', sleep: 5, fatigue: 4, soreness: 4 }] }, '2026-07-19', { nowMinutes: 510 });
          if (!(fDrainCut.pillar === 'focus' && fDrainCut.focusGoalDrained === 40 && fDrainCut.focusTask === null && fDrainCut.focusSlot === null && /focus court et facile aujourd/.test(fDrainCut.insight) && /Lance une session de focus de 25 min maintenant/.test(fDrainCut.action) && !/Cr[ée]neau libre|Reprends « Compta »|enchaîne un bloc/.test(fDrainCut.action))) return false;
          // Contrôle : même état SANS check-in du jour → la poussée de bloc et le créneau reviennent (la coupe ne mord que sur le frein).
          const fDrainCtrl = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-07', minutes: 30, task: 'Compta' }, { date: '2026-07-08', minutes: 30, task: 'Compta' }, { date: '2026-07-09', minutes: 30, task: 'Compta' }, { date: '2026-07-13', minutes: 30, task: 'Compta' }], agenda: [{ id: 'a', date: '2026-07-19', time: '09:00', durationMin: 60 }] }, '2026-07-19', { nowMinutes: 510 });
          if (!(fDrainCtrl.focusGoalDrained === null && fDrainCtrl.focusTask === 'Compta' && fDrainCtrl.focusSlot === '08:30' && /Cr[ée]neau libre à 08:30 aujourd/.test(fDrainCtrl.action))) return false;
          ...
          // Coach CONSCIENT de la RÉGULARITÉ du COUCHER : durée plate + court → sommeil ; coucher qui SE
          // DISPERSE (récente éparpillée vs précédente serrée) → « se disperse » ; qui SE RESSERRE → « se régularise ».
          const brDisp = []; for (let i = 0; i < 7; i++) brDisp.push({ date: '2026-07-' + ('0' + (16 - i)).slice(-2), sleep: 6, bedtime: i % 2 ? '02:00' : '22:00' });
          for (let i = 7; i < 14; i++) brDisp.push({ date: '2026-07-' + ('0' + (16 - i)).slice(-2), sleep: 6, bedtime: '23:30' });
          const fBrDisp = adaptiveCoachFocus({ recovery: brDisp, workouts: stWkDrop }, '2026-07-16');
          if (!(fBrDisp.pillar === 'sommeil' && fBrDisp.sleepTrend === null && fBrDisp.sleepBedtimeTrend > 0 && /ton coucher se disperse/.test(fBrDisp.insight))) return false;
          const brTight = []; for (let i = 0; i < 7; i++) brTight.push({ date: '2026-07-' + ('0' + (16 - i)).slice(-2), sleep: 6, bedtime: '23:30' });
          for (let i = 7; i < 14; i++) brTight.push({ date: '2026-07-' + ('0' + (16 - i)).slice(-2), sleep: 6, bedtime: i % 2 ? '02:00' : '22:00' });
          const fBrTight = adaptiveCoachFocus({ recovery: brTight, workouts: stWkDrop }, '2026-07-16');
          if (!(fBrTight.sleepBedtimeTrend < 0 && /ton coucher se régularise/.test(fBrTight.insight) && !/ton coucher se disperse/.test(fBrTight.insight))) return false;
          // Coach CONSCIENT de la PENTE de FOCUS : pilier focus à corriger + minutes en RECUL → note « minutes de focus
          // reculent » ; focus en renfort + minutes en HAUSSE → note « le volume grimpe » (focusTrend signé, jamais l'inverse).
          const fFocusDown = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-03', minutes: 60 }, { date: '2026-07-04', minutes: 60 }, { date: '2026-07-05', minutes: 60 }, { date: '2026-07-06', minutes: 60 }, { date: '2026-07-07', minutes: 60 }, { date: '2026-07-10', minutes: 30 }, { date: '2026-07-12', minutes: 30 }] }, '2026-07-16');
          if (!(fFocusDown.pillar === 'focus' && fFocusDown.focusTrend < 0 && /minutes de focus reculent/.test(fFocusDown.insight))) return false;
          const fFocusUp = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-03', minutes: 20 }, { date: '2026-07-05', minutes: 20 }, { date: '2026-07-11', minutes: 60 }, { date: '2026-07-12', minutes: 60 }, { date: '2026-07-13', minutes: 60 }, { date: '2026-07-14', minutes: 60 }] }, '2026-07-16');
          if (!(fFocusUp.pillar === 'focus' && fFocusUp.focusTrend > 0 && /le volume grimpe/.test(fFocusUp.insight) && !/reculent/.test(fFocusUp.insight))) return false;
          // Garde-fou §4 ter (#648) : minutes en RECUL + tête À PLAT (readiness < 50 → focusMarginDrained) → la
          // poussée « un bloc aujourd'hui inverse la pente » se TAIT (elle contredirait « un focus léger, une vraie
          // pause suffit »), mais focusTrend reste renvoyé (diagnostic). Pendant, côté note d'insight, du focusRested de #588.
          const fFocusDownRested = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-03', minutes: 60 }, { date: '2026-07-04', minutes: 60 }, { date: '2026-07-05', minutes: 60 }, { date: '2026-07-06', minutes: 60 }, { date: '2026-07-07', minutes: 60 }, { date: '2026-07-10', minutes: 30 }, { date: '2026-07-12', minutes: 30 }], recovery: [{ date: '2026-07-16', sleep: 5, fatigue: 4, soreness: 4 }] }, '2026-07-16');
          if (!(fFocusDownRested.pillar === 'focus' && fFocusDownRested.focusMarginDrained != null && fFocusDownRested.focusTrend < 0 && !/minutes de focus reculent/.test(fFocusDownRested.insight) && !/inverse la pente/.test(fFocusDownRested.insight))) return false;
          // Garde-fou §4 ter (#653) : minutes en RECUL + BLOC DE FOCUS DÉJÀ POSÉ aujourd'hui (07-16) → l'action bascule
          // sur « déjà posé ✅ — savoure » ; la poussée « un bloc aujourd'hui inverse la pente » la contredirait dos à
          // dos → elle se TAIT, focusTrend reste renvoyé (diagnostic). Distinct de #648 (jour de tête à plat).
          const fFocusDownDone = adaptiveCoachFocus({ focusSessions: [{ date: '2026-07-03', minutes: 40 }, { date: '2026-07-04', minutes: 40 }, { date: '2026-07-05', minutes: 40 }, { date: '2026-07-06', minutes: 40 }, { date: '2026-07-14', minutes: 30 }, { date: '2026-07-16', minutes: 30 }] }, '2026-07-16');
          if (!(fFocusDownDone.pillar === 'focus' && fFocusDownDone.focusTrend < 0 && /déjà posé aujourd’hui/.test(fFocusDownDone.action) && !/minutes de focus reculent/.test(fFocusDownDone.insight) && !/inverse la pente/.test(fFocusDownDone.insight))) return false;
        ...
        examListUI: typeof upsertExamGoal === 'function' && typeof removeExamGoal === 'function'
          ...
          try {
            ...
            // CORRECTIF #592 : id = date + slug du titre (deux épreuves le même jour ne fusionnent plus).
            ok = ok && delBtns[0].dataset.delExam === 'exam-2026-07-31-revision-droit' && delBtns[1].dataset.delExam === 'exam-2026-08-15-revision-compta';
      ...
      checks.rubanSurLaMemeFenetre = (() => {
        ...
        try {
          ...
          /* --- PASSE 3 : le ruban s est TU il y a deux mois, la balance non. --- */
          const perimees = [];
          for (let k = 180; k >= 60; k -= 15) perimees.push({ date: ilYA(k),
            waist: Math.round((86 - 2 * (180 - k) / 120) * 10) / 10 });
          state.measurements = perimees;
          const longuePesee = [];
          for (let k = 180; k >= 0; k -= 3) longuePesee.push({ date: ilYA(k), value: 82 });
          state.weights = longuePesee;
          const fVieux = fenetreRecomposition(state.weights, state.measurements,
            "waist", localDate());
          const vuVieux = lire();
          /* TEMOIN : la fenetre doit VRAIMENT etre en retard, sinon la passe ne teste rien. */
          const vraimentPerime = !!fVieux && fVieux.retardJours >= 30 && fVieux.jours > 0;
          /* « les N derniers mois » affirmerait que la mesure court jusqu a aujourd hui. */
          const nePretendPasEtreAJour = !!vuVieux
            && vuVieux.texte.indexOf("derniers mois") === -1
            && vuVieux.texte.indexOf("derni\u00e8re mensuration il y a") !== -1;
          /* Et le nombre de mois annonce est celui de la fenetre, DERIVE, jamais ecrit en dur. */
          const moisFenetre = fVieux ? Math.round(fVieux.jours / 30) : -1;
          const diLaBonneDuree = !!vuVieux
            && vuVieux.texte.indexOf("Sur " + moisFenetre + " mois") !== -1;
          ...
          checks.__ruban = "stable[poids=" + (fStable ? fStable.poidsDelta : "?")
            ...
            + " distinctes=" + passesDistinctes
            + "] perime[retard=" + (fVieux ? fVieux.retardJours : "?")
            + " jours=" + (fVieux ? fVieux.jours : "?") + " vrai=" + vraimentPerime
            + " honnete=" + nePretendPasEtreAJour + " duree=" + diLaBonneDuree
            + "] t1[" + (vuStable ? vuStable.texte.slice(0, 96) : "")
            + "] t3[" + (vuVieux ? vuVieux.texte.slice(0, 108) : "") + "]";

          _rendre();
          return piegeReel && passesDistinctes && ditRecompo && nommeLaPeriode && ditPerte
            && vraimentPerime && nePretendPasEtreAJour && diLaBonneDuree;
      ...
      /* DEUX ECHEANCES DE POIDS, DEUX REGLES, CHACUNE NOMMEE (BLOQUANT, iteration 117 — A4).
         Mesure avant, sur un historique REGULIER de 9 pesees a -0,35 kg/semaine, meme instant :
           ...
           page Poids      : « ≈ 10 semaines · cible estimee le 08/10 (au rythme de 0,55 kg/sem.) »
         Les deux sont HONNETES — la premiere mesure ce que tu FAIS, la seconde annonce ce que ton
         plan calorique VISE — mais aucune ne le disait, et le lecteur voyait deux echeances pour
         un seul objectif, a un facteur 1,4. Meme defaut qu a la 106 (« cette semaine » designait
         deux fenetres), meme remede : chaque voix nomme sa regle. */
      checks.deuxRythmesDeuxRegles = (() => {
        const _w = state.weights, _g = JSON.parse(JSON.stringify(state.goals || {}));
        const _p = JSON.parse(JSON.stringify(state.profile || {}));
        const _tabAvant = (typeof athleteTab === "string") ? athleteTab : "aujourdhui";
        const _rendre = () => { state.weights = _w; state.goals = _g; state.profile = _p;
          try { render(); } catch (_) {} try { showPage("athlete"); showAthleteTab(_tabAvant); } catch (_) {} };
        try {
          const pad = n => String(n).padStart(2, "0");
          const cle = d => d.getFullYear() + "-" + pad(d.getMonth() + 1) + "-" + pad(d.getDate());
          const auj = new Date(localDate() + "T12:00:00");
          ...
          state.profile = Object.assign({}, state.profile, { weight: 80, height: 178, age: 29,
            sex: "homme", activityLevel: "actif", goal: "perte" });
          state.goals = Object.assign({}, state.goals, { targetWeight: 73 });
          /* NEUF pesees hebdomadaires a -0,35 kg : une tendance LENTE, franchement plus lente que
             ce qu un deficit calorique vise. C est ce qui cree le desaccord a nommer. */
          const w = [];
          for (let k = 8; k >= 0; k--) w.push({ date: ilYA(k * 7),
            value: Math.round((81 - (8 - k) * 0.35) * 10) / 10 });
          state.weights = w;
          render();
          ...
          showPage("athlete"); showAthleteTab("corps");
          const tAthlete = lire("weightTrend");
          showPage("poids");
          const tPoids = lire("coachWeightBody");

          /* TEMOIN : les deux ECHEANCES doivent differer, sinon il n y a rien a distinguer et
             n importe quel libelle passerait. On lit les semaines annoncees de chaque cote. */
          const semainesDe = (txt, avant) => {
            const i = txt.indexOf(avant);
            if (i === -1) return null;
            const bout = txt.slice(Math.max(0, i - 12), i);
            const m = bout.match(/([0-9]+)/g);
            return m ? Number(m[m.length - 1]) : null;
          };
          const semAthlete = semainesDe(tAthlete, " sem.");
          const semPoids = semainesDe(tPoids, " semaine");
          const desaccordSeme = semAthlete !== null && semPoids !== null && semAthlete !== semPoids;

          // CHAQUE VOIX NOMME SA REGLE.
          const athleteNomme = tAthlete.indexOf("CE rythme") !== -1;
          const poidsNomme = tPoids.indexOf("rythme VIS") !== -1
            && tPoids.indexOf("par ton plan") !== -1;
          // Et aucune ne se fait passer pour l autre.
          const pasDeConfusion = tAthlete.indexOf("par ton plan") === -1
            && tPoids.indexOf("Tendance r") === -1;

          /* ET CHAQUE VOIX NOMME SA FENETRE (iteration 118). Deux voix parlent du poids sur
             l onglet Athlete, et elles ne repondent pas a la meme question :
               weight-panel   : les SIX DERNIERES PESEES (weightTrend garde slice(-6))
               analysis-panel : la PREMIERE pesee comparee a la derniere, sur tout l historique
             Mesure sur neuf pesees hebdomadaires : -0,36 kg/sem d un cote, -0,35 de l autre, et
             une seule des deux disait sur quoi elle comptait. La roadmap demandait de faire taire
             l analyse ; la mesure dit que les deux informations sont DIFFERENTES et utiles — on
             nomme au lieu de supprimer. Le compte de pesees doit etre le VRAI : avec trois pesees
             la fenetre vaut trois, et annoncer six serait un mensonge de plus. */
          showPage("athlete"); showAthleteTab("corps");
          ...
          checks.__deuxRythmes = "semaines=" + semAthlete + "/" + semPoids
            ...
            + " athleteNomme=" + athleteNomme + " poidsNomme=" + poidsNomme
          ...
          _rendre();
          checks.__errDeuxRythmes = String(e && e.message); return false;
        }
      })();
        ...
        try {
          ...
          const releve = function (txt0, ou) {
            ...
            while ((i = txt.indexOf("/", i + 1)) > 0) {
              ...
              /* On tolère les ESPACES autour du slash : la barre du goal-panel écrit « 2 / 2
                 séances », et un analyseur qui exige des chiffres collés la manquait — la voix que
                 je venais de corriger échappait donc à son propre garde-fou. */
              let a = i - 1, b = i + 1;
              while (a >= 0 && txt.charAt(a) === " ") a--;
              while (b < txt.length && txt.charAt(b) === " ") b++;
              const finGauche = a + 1, debutDroite = b;
          ...
          let borneHaute = null, detailBorne = '';
          {
            ...
            state.workouts = state.workouts.concat([{ id: 105999, date: j(7), type: 'strength',
              duration: 45, effort: 2, exercises: [{ name: 'Dips', sets: 3, reps: 10, completedSets: 3 }] }]);
            const apres = (typeof thisWeekWorkouts === 'function') ? thisWeekWorkouts().length : null;
            borneHaute = (avant !== null && apres === avant);
            detailBorne = avant + '->' + apres;
            state.workouts = state.workouts.filter(function (w) { return w.id !== 105999; });
            render();
          }

          /* LE LIBELLE DU GRAND CHIFFRE. Le panneau Volume compte TOUTE activite (velo, mobilite),
             ce qui est son sujet — mais place au-dessus d un verdict sur le plan, il se lit comme le
             compte du plan. Deux nombres differents sont honnetes s ils annoncent chacun leur regle. */
          let libelleQualifie = null, detailLibelle = '';
          {
            showAthleteTab('progres');
            const gros = document.getElementById('weekSessions');
            const lbl = document.querySelector('[data-week-sessions-label]');
            const c = (typeof cibleHebdo === 'function') ? cibleHebdo() : null;
            if (gros && lbl && c) {
      ...
      /* LE PLAN DE BATAILLE A ABSORBE « TA PROCHAINE SEANCE » (BLOQUANT, iteration 99).
         ...
         absorbe ce qu il faisait MIEUX. Trois choses, et la premiere est une CORRECTION :

         1. L UNITE DES SERIES. Le Plan ecrivait sets×reps brut : mesure a l appui, « Equilibre
            unipodal 3×30 » pour 30 SECONDES de tenue, « Bear crawl 3×20 » pour 20 PAS. Trente
            repetitions d equilibre unipodal n existent pas — la consigne etait infaisable. Le
            panneau masque, lui, passait par formatFor, qui suffixe l unite.
         2. LE POURQUOI DES SEANCES DE MUSCU. Les courses portaient toutes leur why, les muscu
            aucun : le gabarit ne lisait why que dans la branche course.
         3. LE GESTE « PREPARER » : noter une seance a la main, sans mode guide. Il n existait que
            dans le panneau masque.

         Le fixture est FORCE (objectif athletique, seed 0) : sans lui, le check ne verrait des
         unites non-reps que les jours ou le plan du moment en contient — « une valeur vraie ce
         jour-la » au lieu de son sujet. */
      checks.uniteEtGestesDuPlan = (() => {
        const _objAvant = state.fitnessObjective, _seedAvant = state.objectiveSeed;
        const _tabAvant = (typeof athleteTab === 'string') ? athleteTab : 'aujourdhui';
        const _rendre = () => {
          state.fitnessObjective = _objAvant; state.objectiveSeed = _seedAvant;
        ...
        try {
          ...
          showPage('athlete'); showAthleteTab('aujourdhui');
          const sel = document.getElementById('objectiveSelect');
          const gen = document.getElementById('objectiveGenerate');
          if (!sel || !gen) { _rendre(); return false; }
          state.objectiveSeed = 0;
          sel.value = 'athletique';
          gen.click();

          const lignes = Array.prototype.slice.call(
            document.querySelectorAll('#objectiveResult .op-ex li'));
          const formats = lignes.map(function (li) {
            const em = li.querySelector('em');
            return String((em || {}).textContent || '').trim();
          });
          // Toute ligne doit porter une unite connue. Une ligne « 3×30 » nue est le defaut exact.
          const unites = ['reps', 'sec', 'pas'];
          const nues = formats.filter(f => !unites.some(u => f.indexOf(u) !== -1));
          const nonReps = formats.filter(f => f.indexOf('sec') !== -1 || f.indexOf('pas') !== -1);

          // Chaque jour de muscu explique son placement dans la semaine.
          const joursMuscu = Array.prototype.slice.call(
            document.querySelectorAll('#objectiveResult .op-day.op-muscu'));
          const avecPourquoi = joursMuscu.filter(function (d) {
          ...
          const start = document.querySelectorAll('#objectiveResult [data-op-start]');
          let dialogueOuvert = false, notePrefixee = '';
          const dlg = document.getElementById('workoutDialog');
          if (prep[0] && dlg) {
            prep[0].click();
            dialogueOuvert = dlg.open === true;
            const notes = document.getElementById('workoutNotes');
            notePrefixee = String((notes || {}).value || '').slice(0, 60);
            try { dlg.close(); } catch (_) {}
          }
          const prepareAgit = dialogueOuvert && notePrefixee.indexOf('Séance guidée') !== -1;
          ...
          _rendre();
          checks.__errUniteGestes = String(e && e.message); return false;
        }
      })();
      ...
      checks.generateursMasques = (() => {
        try {
          ...
          if (typeof showAthleteTab === 'function') showAthleteTab('aujourdhui');
          const cibles = [
            /* AJOUTE A L ITERATION 99 : le panneau « Ta prochaine seance » etait le QUATRIEME
               generateur de seances, resté visible quand les trois autres ont ete masques (53). Son
               contenu utile est desormais dans le Plan de bataille — unite des series, pourquoi par
               seance de muscu, geste « Preparer ». Present dans le DOM, masque a l ecran. */
            ['.program-panel', document.querySelector('.program-panel')],
    ...
    if (!checks.weeklyInsights) errors.push('Bilan hebdo intelligent KO (weeklyInsights absent OU félicite « montes en volume » alors que la charge est en pic → allège OU « km courus » au pluriel fautif sous 2 km)');
    ...
    if (!checks.ultraDownhill) errors.push('Préparation descentes KO (downhillPrep : phases base/specific/race, 1-2 séances/sem, STOP à J≤10 — ou #ultraDownhill absent du panneau ultra)');
    if (!checks.kmRamp) errors.push('Montée de kilométrage KO (weeklyKmRamp : zone build/high + champ onBase = base ≥ 10 km — un « high » sur base minuscule doit avoir onBase false)');
    if (!checks.runRampSoften) errors.push('Bannière montée de course KO (renderRunWeekGoal : un « +125 % » sur base < 10 km doit afficher « tu bâtis ta base », pas « hausse forte / risque de blessure » ; le vrai pic sur base ≥ 10 km garde l’alerte)');
    if (!checks.zfRestFallback) errors.push('Fraîcheur musculaire KO (#zoneFreshness : quand des groupes ne sont JAMAIS travaillés, la synthèse doit dire « Prêt aujourd’hui : … » et surtout PAS « journée légère » — sinon elle contredit « À privilégier : … jamais travaillé » juste en dessous)');
    if (!checks.weekLoadNote) errors.push('Charge de la semaine KO (#weekLoadAdvice : un jour de récup basse ET d’ACWR bas doit dire « garde la prochaine séance facile » sans « tu peux remonter progressivement » — sinon repos↔pousse se contredisent dans la même phrase)');
    ...
    if (!checks.a11yObjective) errors.push('A11y panneaux générés KO (#objectiveSelect doit avoir un aria-label ; #objectiveResult / #runPlanResult / #quickSessionResult / #wpResult doivent avoir aria-live=polite — sinon le résultat généré au clic reste silencieux au lecteur d\'écran)');
    ...
    if (!checks.coachWeight) errors.push('Coach Poids KO (energyPlan : bmr/tdee/goal/weeks/targetDate/proteinG attendus — valeurs sourcées #595)');
