  ...
  try {
    ...
    const checks = await win.webContents.executeJavaScript(`(function(){
      return {
        ...
        strengthRecords: typeof strengthRecords === 'function' && !!document.getElementById('strengthRecords') && (() => { const r = strengthRecords([{ date: '2026-06-08', exercises: [{ name: 'DC', load: 45, reps: 5 }, { name: 'Sq', load: 60, reps: 10 }] }]); return r.length === 2 && r[0].name === 'Sq' && r[0].e1rm === 80 && strengthRecords([]).length === 0; })(),
        ...
        bodyGoals: typeof bodyGoalWorkout === 'function' && Array.isArray(BODY_GOALS) && BODY_GOALS.length === 4 && document.querySelectorAll('#bodyGoalsBar [data-bodygoal]').length === 4 && (() => { const w = bodyGoalWorkout('abs', exercises, { count: 5 }); return w && w.title === 'Abdos béton' && w.exercises.length >= 2 && w.exercises.every(e => exerciseZones(e.name).includes('abs') && e.sets > 0) && bodyGoalWorkout('legs', exercises).exercises.every(e => exerciseZones(e.name).some(z => z === 'legs' || z === 'glutes')); })(),
