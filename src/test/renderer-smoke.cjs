  ...
  try {
    ...
    const checks = await win.webContents.executeJavaScript(`(function(){
      return {
        ...
        wellness: typeof wellnessRoutine === 'function' && Array.isArray(WELLNESS_ROUTINES) && WELLNESS_ROUTINES.length >= 5 && !!document.getElementById('wellnessBar') && document.querySelectorAll('#wellnessBar [data-wellness]').length >= 5 && (() => { const r = wellnessRoutine('cooldown'); return r && r.exercises.length >= 3 && r.exercises.every(e => e.unit === 'sec') && wellnessRoutine('zzz') === null; })(),
        replayOnboarding: !!document.getElementById('settingsReplayOnboarding'),
        onboardingSetup: typeof onboardingSetup === 'function' && !!document.getElementById('onboardingObjective') && !!document.getElementById('onbWeight') && !!document.getElementById('onbSex') && !!document.getElementById('finishOnboarding') && (() => { const s = onboardingSetup({ weight: 80, height: 178, age: 30, sex: 'homme', objective: 'muscle', sessions: 4, equipment: { kettlebell: true } }); return s.fitnessObjective === 'muscle' && s.profile.weight === 80 && s.goals.sessions === 4 && s.profile.equipment.kettlebell === true; })(),
