...
function settings() { try { return JSON.parse(fs.readFileSync(settingsFile(), 'utf8')); } catch { return { enabled: false, times: ['09:00', '18:00'], lastSent: {} }; } }
...
function checkReminders() { const cfg = settings(); if (!cfg.enabled) return; const now = new Date(), time = now.toTimeString().slice(0, 5), date = now.toISOString().slice(0,10); if (cfg.times.includes(time) && cfg.lastSent[time] !== date) { cfg.lastSent[time] = date; saveSettings(cfg); showReminder('⚡ Level Up IRL', 'Une mini-quête t’attend. Fais un pas, gagne de l’XP.'); } }
function createWindow() { win = new BrowserWindow({ width: 1120, height: 820, minWidth: 720, minHeight: 620, backgroundColor: '#0d1220', autoHideMenuBar: true, icon: path.join(__dirname, 'assets', 'icon.ico'), webPreferences: { preload: path.join(__dirname, 'preload.cjs'), contextIsolation: true, nodeIntegration: false } }); win.loadFile('index.html'); win.on('close', event => { if (!app.isQuitting) { event.preventDefault(); win.hide(); showReminder('Level Up IRL', 'L’application reste active pour tes rappels.'); } }); }
...
ipcMain.handle('notifications:save', (_, value) => { const cfg = { enabled: Boolean(value.enabled), times: (value.times || []).filter(Boolean).slice(0,3), lastSent: settings().lastSent || {} }; saveSettings(cfg); if (cfg.enabled) showReminder('Rappels activés', 'Tes rendez-vous avec toi-même sont programmés.'); return cfg; });
