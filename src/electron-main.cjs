...
function checkEventReminders(cfg, now, date) { const s = readBackupState(); if (!s) return false; const lead = Math.min(60, Math.max(5, Number(cfg.leadMinutes) || 15)); let changed = false; (Array.isArray(s.agenda) ? s.agenda : []).filter(a => a && a.date === date && !a.completed && isTime(a.time)).forEach(a => { const [h, m] = a.time.split(':').map(Number); const eventAt = new Date(now); eventAt.setHours(h, m, 0, 0); const diff = (eventAt - now) / 60000; const key = `evt-${a.id}-${date}`; if (diff > 0 && diff <= lead && cfg.lastSent[key] !== date) { cfg.lastSent[key] = date; changed = true; showReminder(`🔔 ${a.time} · ${String(a.title || 'Bloc').slice(0, 80)}`, `Dans ${Math.max(1, Math.round(diff))} min. Prépare-toi tranquillement.`); } }); return changed; }
/* Rappel du soir : s'il reste des blocs ou des quêtes non faits. */
function checkEveningReminder(cfg, time, date) { if (cfg.eveningEnabled === false) return false; const evening = isTime(cfg.eveningTime) ? cfg.eveningTime : '21:00'; if (time !== evening || cfg.lastSent[`evening-${date}`] === date) return false; const s = readBackupState(); if (!s) return false; const remaining = (Array.isArray(s.agenda) ? s.agenda : []).filter(a => a && a.date === date && !a.completed).length; const quests = (Array.isArray(s.quests) ? s.quests : []).filter(q => q && !q.done).length; if (!remaining && !quests) return false; cfg.lastSent[`evening-${date}`] = date; const parts = []; if (remaining) parts.push(`${remaining} bloc${remaining > 1 ? 's' : ''}`); if (quests) parts.push(`${quests} quête${quests > 1 ? 's' : ''}`); showReminder('🌙 Fermer la journée', `Encore ${parts.join(' et ')} aujourd’hui. Un petit geste suffit, puis repos.`); return true; }
