...
function adaptiveCoachFocus(state, todayKey, opts) {
  ...
  if (chosen.pillar === 'sommeil' && sleepIns) {
    ...
    if (sleepTrend === null && typeof bedtimeRegularityTrend === 'function') {
        ...
        if (typeof weightTrend === 'function') {
          ...
          if (wt) {
            ...
            if (wt.onTrack === true && wt.weeksToTarget != null && wt.weeksToTarget > 0) {
              ...
              if (typeof energyPlan === 'function' && typeof calorieAdjustment === 'function') {
  ...
  if (!rotated && tone === 'reinforce' && typeof coachFollowThrough === 'function') {
    ...
    if (ft && ft.total >= 3 && ft.rate >= 70) {
      ...
      // On garde le crédit du suivi (insight), mais on NE réécrit PAS l'action quand une alerte de charge
      // (loadSpike) tient déjà la barre : « un jour actif de plus » contredirait « allège aujourd'hui ».
      if (loadSpike == null) action = 'Un jour actif de plus aujourd’hui : tu prouves que la régularité te ressemble.';
    }
  }
  ...
  return {
    ...
    lastActiveDays: chosen.lastActiveDays, headline, insight, action, rotated, microStep, followThrough, readiness, focusTask, focusBlockMin, focusSlot, sportSlot, sleepConflict, sleepConflictBedtime, reviveStep, comeback, comebackStage, doneToday, alsoSlipping, alsoSlippingPillars, pillarsToday, completeDayStreak, completeDayMilestone, streakAtRisk, streakMilestoneReach, streakRecordReach, streakRebuild, brokenStreak, brokenStreakTier, weightGoalPct, weightPace, calorieTarget, sessionGoalPace, loadSpike, readinessSlide, readinessRebound, lowLoad, sleepTrend, sleepBedtimeTrend, focusTrend, proteinTrend,
  };
}
...
// Pente d'ADHÉRENCE protéines — le pendant, côté NUTRITION façon INTRANT, de `focusMinutesTrend`.
// proteinStreak dit la série EN COURS et proteinDaysOnTarget le compte PONCTUEL de la semaine ; aucun
// des deux ne voit la DIRECTION : la régularité protéines monte-t-elle ou s'effrite-t-elle d'une
// semaine à l'autre ? On compte les jours où la protéine du jour (agrégée au MAX par date, comme
...
function proteinAdherenceTrend(nutrition, target, todayKey, windowDays) {
  const t = Math.round(Number(target) || 0);
  const isKey = k => /^\d{4}-\d{2}-\d{2}$/.test(String(k || ''));
  if (!isKey(todayKey) || t <= 0) return null;
  const w = Math.max(2, Math.min(60, Math.round(Number(windowDays) || 7)));
  const byDate = {};
  (Array.isArray(nutrition) ? nutrition : []).forEach(n => {
    if (n && isKey(n.date)) byDate[n.date] = Math.max(byDate[n.date] || 0, Number(n.protein) || 0);
  });
  const pad = n => String(n).padStart(2, '0');
