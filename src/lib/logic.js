...
// Répartition des candidatures par étape du pipeline, pour une barre visuelle (funnel).
// Ordre de progression. 'refus' est renvoyé À PART (sortie du pipeline) — pas dans la barre ni le `sum`.
const FUNNEL_STAGES = [
  { key: 'a_postuler', label: 'À postuler', color: '#9da8bd' },
  { key: 'postule', label: 'Postulé', color: '#5e9df0' },
  { key: 'relance', label: 'Relancé', color: '#e0a83a' },
  { key: 'entretien', label: 'Entretien', color: '#a78bfa' },
  { key: 'accepte', label: 'Accepté', color: '#abff55' },
];
function applicationFunnel(stats) {
  const bs = (stats && stats.byStatus && typeof stats.byStatus === 'object') ? stats.byStatus : {};
  const stages = FUNNEL_STAGES.map(s => ({ key: s.key, label: s.label, color: s.color, count: Math.max(0, Math.round(Number(bs[s.key]) || 0)) }));
  const sum = stages.reduce((a, s) => a + s.count, 0);
  const refus = Math.max(0, Math.round(Number(bs.refus) || 0));
  return { stages, sum, refus };
}
...
function jobStatusFromText(t) {
  ...
  // bascule en « Accepté 🎉 » et gonfle applicationStats (accepted, entretiens) à CHAQUE sync (miroir
  // exact du garde `retenu` ci-dessus, resté asymétrique jusqu'ici). On n'attrape QUE la tournure avec
  // AUXILIAIRE (`été/suis/est… pris`), pas le « pris » nu : « pas encore pris contact » reste une
  // prospection, pas un refus.
...
function acuteChronicRatio(workouts, todayKey) {
  ...
  // Sans AUCUNE charge HORS de la fenêtre aiguë (jours 7-27), il n'existe pas de base chronique
  // distincte : chronic28 === acute → ratio = acute/(acute/4) = 4,0, FIXÉ par l'arithmétique et non
  // par un vrai risque. C'est typiquement la 1re semaine d'entraînement (aucun historique) : l'ACWR y
  // est indéfini. On renvoie donc null (même contrat que « pas de charge chronique ») pour que les
  // consommateurs retombent sur leur défaut neutre au lieu d'un faux « pic de charge / allège »
  // (blessure) qui découragerait un débutant. Une seule séance datée d'avant les 7 j suffit à amorcer
  // la base et à ré-activer le calcul (cf. le pic à base faible, déjà couvert et testé).
  if (!(chronic28 > acute)) return null;
...
// Ligne « charge de la semaine » (#weekLoadAdvice) : réconcilie DEUX signaux indépendants — le verdict
// RÉCUP du jour (`fragile` = dernier check-in sous les seuils sommeil/fatigue/courbatures) et le ratio
// ACWR aigu/chronique (`acwr` = sortie d'`acuteChronicRatio`, ou null). Sans arbitrage, `fragile` + zone
// `low` affichait « garde la prochaine séance facile » PUIS « charge en baisse — tu peux remonter
// progressivement » dans la même phrase : contradiction repos↔pousse. La récup PRIME (même logique que
// `loadAdvice`, qui fait passer la forme basse avant la charge basse) : sous fatigue, la charge basse
// n'est plus une invitation à remonter tout de suite mais un « remonte quand la forme sera revenue ».
// Le ratio chiffré reste affiché (diagnostic factuel). Pur + testé.
function weekLoadNote(fragile, load, acwr) {
  let note = fragile ? 'Récupération basse : garde la prochaine séance facile ou raccourcie.'
    : load > 500 ? 'Semaine dense : protège au moins une vraie journée légère.'
    : load ? 'Charge progressive : conserve une marge sur les séances dures.'
    : 'La charge apparaîtra après ta première séance.';
  if (acwr) {
    const zoneTxt = acwr.zone === 'high' ? '⚠️ pic de charge — risque accru, allège cette semaine'
      : acwr.zone === 'low' ? (fragile ? 'charge en baisse — remonte quand la forme sera revenue' : 'charge en baisse — tu peux remonter progressivement')
      : '✅ zone optimale';
    note += ` · Aiguë/chronique ${acwr.ratio} (${zoneTxt}).`;
  }
  return note;
}
...
function studyPacing(agenda, examGoal, todayKey) {
  ...
  // CORRECTIF #680 : `remaining === 0` = « plus rien à réviser d'ici l'examen » — mais PAS forcément
  // « tout fait » : des séances EN SCOPE passées et NON faites comptent dans `total` sans être dans
  // `done`. L'ancien raccourci renvoyait alors `status:'done'` → la carte pacing célébrait
  // « 🎉 Toutes tes révisions planifiées sont faites » alors que `done < total` (aucune validée),
  // pendant qu'`overdueStudy` listait ces mêmes séances comme « N révisions en retard » juste en
  // dessous : deux cartes qui se contredisaient. On ne célèbre 'done' que si des révisions étaient
  // prévues ET toutes complétées ; sinon (séances manquées, ou aucune séance en scope pour l'épreuve
  // proche — cas multi-épreuves `total === 0`) → null : la carte s'efface, `overdueStudy` porte le
  // message honnête. Le cas légitime « tout fait » (`done === total > 0`) reste célébré.
  if (remaining === 0) return (total > 0 && done === total) ? { remaining: 0, daysLeft: c.daysLeft, perWeek: 0, done, total, status: 'done' } : null;
...
// AFFÛTAGE (taper) avant une course objectif — fondé sur la méta-analyse de référence.
// Bosquet et al. 2007 (Med Sci Sports Exerc, méta de 27 études) : sur les ~2 dernières semaines,
// RÉDUIRE le VOLUME de 41-60 % (≈ le diviser par deux) en COUPANT LA DURÉE des séances, tout en
// GARDANT l'intensité (allure course / VO2max) ET la fréquence (nb de séances) — baisser l'intensité
// n'apporte rien, baisser la fréquence non plus. Une décroissance EXPONENTIELLE (progressive) du
// volume bat un palier sec (Mujika & Padilla 2003). Gain attendu ~2-4 %. La durée d'affûtage s'allonge
// avec la distance (fatigue accumulée à évacuer) : ~1 sem sur 10 km, ~1,5-2 sem sur semi/marathon,
// ~2,5 sem en ultra/trail. `daysLeft` = jours avant la course, `raceKm` = distance. Renvoie null hors
// fenêtre d'affûtage (course lointaine ou passée), sinon le multiplicateur de volume + repères. Pur + testé.
// Durée d'affûtage (jours) selon la distance de course (Bosquet ~2 sem ; échelonnée par distance,
// Mujika 2003). SOURCE UNIQUE de la fenêtre d'affûtage : `taperPlan` ET `downhillPrep` s'y réfèrent,
// pour qu'« arrive frais » et « plus de descente cassante » se déclenchent au même moment (sinon
// contradiction dans la fenêtre J-11..J-18 pour les longues distances). Pur + testé.
function taperDaysFor(raceKm) {
  const km = Number(raceKm) || 0;
  return km <= 12 ? 7 : km <= 30 ? 11 : km <= 45 ? 14 : 18;
}

function taperPlan(daysLeft, raceKm) {
  if (daysLeft == null || daysLeft === '') return null;     // pas de course (app.js passe null) → jamais d'affûtage
  const d = Number(daysLeft);
  if (!Number.isFinite(d) || d < 0) return null;            // course passée ou date invalide
  const km = Number(raceKm) || 0;
  const taperDays = taperDaysFor(km);
  if (d > taperDays) return null;                            // pas encore en affûtage
  // Décroissance EXPONENTIELLE du volume (Bosquet 2007 : progressive > palier). p = 0 à l'entrée de
  // l'affûtage, 1 le jour J. On entre en douceur (~15 % de coupe) et on descend vers ~50 % près de la
  // course, en restant dans la bande optimale 41-60 %.
  const p = 1 - d / taperDays;
  const startMul = 0.85, floorMul = 0.45;
  const volumeMul = Math.round((floorMul + (startMul - floorMul) * Math.exp(-2.3 * p)) * 100) / 100;
  const cutPct = Math.round((1 - volumeMul) * 100);
  const raceWeek = d <= 7;
  const label = raceWeek ? 'Affûtage · semaine de course' : 'Affûtage · dernières semaines';
  const note = raceWeek
    ? `Semaine de course (J-${d}) : réduis le VOLUME d’environ ${cutPct} % — des sorties plus COURTES — mais garde de l’allure course sur de brefs rappels et le même nombre de séances. Arrive frais, dors et mange bien (Bosquet 2007).`
    : `Affûtage (J-${d}) : commence à alléger le VOLUME (~${cutPct} %) en raccourcissant tes séances, sans toucher à l’intensité ni au nombre de sorties. La forme se révèle en réduisant la fatigue, pas en s’entraînant plus (Bosquet 2007).`;
  return { taperDays, daysLeft: d, volumeMul, cutPct, raceWeek, label, note, source: 'Bosquet 2007' };
}

// Préparation aux DESCENTES — le stress trail que personne n'entraîne. Courir en descente est un
// travail EXCENTRIQUE qui inflige des dommages musculaires (DOMS, perte de force, CK élevée) : c'est
// ce qui « casse les jambes » sur un trail vallonné, bien plus que la montée. La parade est validée :
// l'EFFET DE SÉANCE RÉPÉTÉE (repeated bout effect) — une seule séance de descente protège durablement
// contre une séance ultérieure (Chen & Nosaka ; Frontiers Physiol. 2018 ; ~4 sem/10 séances →
// adaptations neuromusculaires de type excentrique). Reco : ≥ 1 séance de descente dans les semaines
// qui précèdent une course off-road, SURTOUT pour qui n'est pas habitué aux pentes négatives — MAIS
// PLUS DÈS L'ENTRÉE EN AFFÛTAGE (la protection se construit tôt ; on n'ajoute pas de dommage frais
// avant le jour J). Le seuil de coupe est aligné sur `taperDaysFor` (plancher 10 j) : sinon, sur une
// longue distance dont l'affûtage démarre à J-11/14/18, l'app dirait « arrive frais » (taperPlan) ET
// « 2 descentes/sem, ça casse les jambes » dans la même fenêtre — la contradiction que ceci corrige.
// Entrées : `dplusPerWeek` = D+ hebdo réel ou visé (m) — sert de signal « profil trail » ;
// `raceDaysLeft` = jours avant la course objectif (ou null), `raceKm` = distance. Renvoie
// { sessionsPerWeek, window, protocol, why, source } ou null hors contexte trail (aucun D+). Pur + testé.
function downhillPrep(dplusPerWeek, raceDaysLeft, raceKm) {
  const vert = Number(dplusPerWeek) || 0;
  if (vert <= 0) return null;                                 // pas de dénivelé = pas de contexte trail
  const km = Number(raceKm) || 0;
  const d = (raceDaysLeft == null || raceDaysLeft === '') ? null : Number(raceDaysLeft);
  const hasRace = d != null && Number.isFinite(d) && d >= 0;
  const source = 'effet de séance répétée · descentes (Frontiers Physiol. 2018)';
  // Dès l'entrée en affûtage (au plus tard J-10) : STOP aux descentes cassantes — la protection est
  // déjà acquise, on préserve la fraîcheur. Le seuil suit `taperDaysFor` pour rester COHÉRENT avec
  // `taperPlan` sur les longues distances (affûtage J-11/14/18), avec un plancher de sécurité à 10 j.
  const stopDays = Math.max(10, taperDaysFor(km));
  if (hasRace && d <= stopDays) {
    return {
      sessionsPerWeek: 0, window: 'race', source,
      protocol: `À J-${d}, plus de séance de descente « cassante » : la protection excentrique est déjà en place et l’affûtage a commencé. Garde des descentes souples et courtes, jambes fraîches pour le jour J.`,
      why: 'Une séance de descente à l’approche laisserait des jambes endolories le jour de la course — la protection se construit des semaines avant, pas la veille.'
    };
  }
  // Fenêtre spécifique (course à ≤ 8 sem) : monter à 1-2 séances/sem selon le profil vallonné.
  const specific = hasRace && d <= 56;
  const sessionsPerWeek = specific ? (vert >= 1000 ? 2 : 1) : 1;
  const window = specific ? 'specific' : 'base';
  const protocol = `${sessionsPerWeek} séance${sessionsPerWeek > 1 ? 's' : ''}/sem : sur une pente douce (5-10 %), 4-6 × 60-90 s en descente CONTRÔLÉE (foulée courte, cadence haute, freine sans bloquer le genou), remontée en marchant. Ajoute une répétition par semaine.`;
  const why = specific
    ? `Ta course approche : ${km ? `${km} km ` : ''}de descente inflige un stress excentrique qui casse les jambes. Une descente régulière déclenche l’effet de séance répétée et te protège le jour J.`
    : 'Habitue tes jambes aux descentes toute l’année : l’adaptation excentrique protège durablement contre les dommages musculaires en trail.';
  return { sessionsPerWeek, window, protocol, why, source };
}
...
function warmupFor(title) {
  ...
  if (/poussée|tirage|\bhaut\b|\btraction|pompes|\bpress\b|militaire/.test(t))
...
// PREHAB / prévention des blessures, niveau kiné, spécifique à la zone travaillée. Fondé science :
// l'ajout d'un renforcement ciblé — surtout EXCENTRIQUE — réduit d'environ MOITIÉ le risque de blessure
// (Lauersen et al. 2014, Br J Sports Med 48:871, méta-analyse ; ~2/3 pour l'excentrique, Nordic
// hamstring). Renvoie { label, why, moves } — quelques mouvements concrets à insérer dans la séance.
// Pur + testé.
function prehabFor(title) {
  const t = String(title || '').toLowerCase();
  if (/poussée|tirage|\bhaut\b|\btraction|pompes|\bpress\b|militaire/.test(t))
    return { label: 'Prévention épaule · ~3 min', why: 'Renforcer la coiffe des rotateurs protège l’épaule des pousseurs (Lauersen 2014).', moves: ['Rotations externes à l’élastique · 2×15/bras (léger, contrôlé)', 'Face pulls · 2×15 (omoplates serrées, coudes hauts)', 'YTW au sol ou buste penché · 2×8 par lettre', 'Étirement pectoral 30 s/côté avant de charger'] };
  if (/jambe|chaîne|squat|fessier|fente|mollet|cuisse|bas du corps/.test(t))
    return { label: 'Prévention genou & ischios · ~4 min', why: 'Le renforcement excentrique des ischios réduit fortement les blessures (Nordic hamstring, Lauersen 2014).', moves: ['Nordic hamstring curl · 2×5 (descente FREINÉE 3-4 s, aide-toi des mains en bas)', 'Pont fessier (glute bridge) · 2×15, 2 s de pause en haut', 'Élévations de mollet lentes · 2×15 (descente contrôlée, tendon d’Achille)', 'Gainage latéral · 2×20-30 s/côté (stabilité hanche/genou)'] };
  if (/trail|côte|course|puissance|longue|swing|explos|prévention/.test(t))
    return { label: 'Prévention coureur · ~4 min', why: 'Excentrique mollets + ischios et gainage latéral préviennent périostite, tendinite d’Achille et blessures d’ischios (Lauersen 2014).', moves: ['Élévations de mollet lentes · 2×15/jambe (descente freinée 3 s)', 'Nordic hamstring curl · 2×5 (descente FREINÉE)', 'Gainage latéral + montée de hanche · 2×10/côté', 'Équilibre unipodal yeux fermés · 2×20 s/jambe (proprioception cheville)'] };
  return { label: 'Prévention & gainage · ~3 min', why: 'Un gainage et une mobilité réguliers protègent le dos et les articulations (Lauersen 2014).', moves: ['Planche · 2×20-30 s (bassin gainé, fessiers serrés)', 'Bird-dog (chien d’arrêt) · 2×8/côté (dos neutre)', 'Pont fessier · 2×15', 'Mobilité hanche + cheville · 30 s/côté'] };
}
...
// Dates réelles où poser un programme d'entraînement, ANCRÉES sur la semaine EN COURS (lundi→dimanche)
// et non sur lundi prochain. `days` = [{weekday}] (1 = lundi … 7 = dimanche, cf. buildTrainingWeek).
// On part du lundi de la semaine de `todayKey`, on couvre `weeks` semaines, et on SAUTE les jours déjà
// PASSÉS de la semaine courante (date < todayKey) : ainsi la semaine en cours reçoit déjà ses séances
...
function weekProgramSchedule(days, todayKey, weeks) {
  ...
  const out = [];
  for (let k = 0; k < w; k++) {
    list.forEach((d, dayIndex) => {
      const wd = Number(d && d.weekday);
      if (!(wd >= 1 && wd <= 7)) return;
      const day = new Date(monday);
      day.setDate(day.getDate() + k * 7 + (wd - 1));
      const date = dateKey(day);
      if (date < todayKey) return; // jour déjà écoulé de la semaine en cours → on ne le (re)pose pas
      out.push({ date, weekday: wd, dayIndex, week: k });
    });
  }
...
function weeklyAggregate(records, options) {
  ...
  (Array.isArray(records) ? records : []).forEach(r => {
    // isRealDateKey exige une date calendaire réelle : une date malformée (2026/07/15, 20260715) ou
    // impossible (2026-13-40, 2026-02-30) fabriquerait une Invalid Date → dateKey().toISOString() jetterait
    // une RangeError qui remonte jusqu'à renderCharts et casse tout le dashboard. On l'ignore comme une
    // date hors fenêtre (§ robustesse : une donnée héritée/importée douteuse ne doit pas planter le rendu).
...
// Recommandation de DÉCHARGE (deload) muscu fondée sur le VOLUME réel logué. Renaissance Periodization /
// Israetel (mésocycle : accumuler de MEV≈10 vers MRV≈20 séries/muscle/sem sur ~4-6 sem, PUIS décharger
// 5-7 j) et Helms/Zourdos (décharge toutes les 4-6 sem OU sur signaux de fatigue) : une semaine de
// décharge tombe le volume à ~50-60 % (soit −40 à −50 % de séries), garde charges et technique, et le
// corps surcompense. Analyse les séries hebdo (toutes zones) sur `weeksBack` semaines glissantes
// (défaut 6) : compte les semaines DURES d'affilée parmi les semaines COMPLÈTES (hors semaine en cours,
// incomplète) et déclenche quand l'accumulation atteint `hardWeeksTrigger` (défaut 5), ou plus tôt si la
// forme du jour est basse (readiness < 45) après ≥ trigger-2 semaines dures. Une semaine « dure » = volume
// total ≥ 55 % du pic ET ≥ 15 séries (un vrai bloc de charge) ; une semaine légère/de repos casse le
// compteur (= décharge déjà prise). Renvoie { due, reason:'accumulation'|'fatigue'|null, hardWeeks,
// peakZone, peakSets, overMrv, weeks, advice, emoji } ou null si trop peu d'historique (< 2 semaines avec
// des séries). Display-only : n'alimente aucun compteur. Pur + testé.
function deloadRecommendation(workouts, todayKey, opts) {
  const t = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(todayKey || ''));
  if (!t) return null;
  const o = opts && typeof opts === 'object' ? opts : {};
  const weeksBack = Math.max(3, Math.round(Number(o.weeksBack) || 6));
  const trigger = Math.max(3, Math.round(Number(o.hardWeeksTrigger) || 5));
  const today = new Date(+t[1], +t[2] - 1, +t[3]); today.setHours(0, 0, 0, 0);
  const list = Array.isArray(workouts) ? workouts : [];
  // week[i] = séries par zone sur les jours [i*7 .. i*7+6] avant aujourd'hui (i=0 = semaine en cours).
  const weekZones = [];
  for (let i = 0; i <= weeksBack; i++) weekZones.push({});
  list.forEach(w => {
    const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(w && w.date || '')); if (!m) return;
    const d = new Date(+m[1], +m[2] - 1, +m[3]); d.setHours(0, 0, 0, 0);
    const days = Math.round((today - d) / 86400000);
    ...
    exos.forEach(ex => {
      ...
      const s = Number(ex.completedSets) > 0 ? Number(ex.completedSets) : (Number(ex.sets) || 0);
      if (s <= 0) return;
      exerciseZones(ex.name).forEach(z => { weekZones[wi][z] = (weekZones[wi][z] || 0) + s; });
    });
  });
  const totals = weekZones.map(z => Object.keys(z).reduce((a, k) => a + z[k], 0));
  if (totals.filter(x => x > 0).length < 2) return null;
  const peak = Math.max.apply(null, totals);
  const isHard = tot => tot >= Math.max(15, peak * 0.55);
  // Accumulation : semaines dures consécutives parmi les COMPLÈTES (indices 1..weeksBack) — on exclut
  // la semaine en cours (indice 0), incomplète, pour ne pas sous-compter en début de semaine.
  let hardWeeks = 0;
  for (let i = 1; i <= weeksBack; i++) { if (isHard(totals[i])) hardWeeks++; else break; }
  // Zone la plus chargée récemment (semaine en cours si elle a des séries, sinon dernière complète).
  const peakSrc = totals[0] > 0 ? weekZones[0] : weekZones[1];
  let peakZone = null, peakSets = 0;
  Object.keys(peakSrc || {}).forEach(z => { if (peakSrc[z] > peakSets) { peakSets = peakSrc[z]; peakZone = z; } });
  const overMrv = peakSets > 20;
  // readiness accepté en NOMBRE (score 0-100) OU en objet renvoyé par readinessScore ({score,label}) :
  // c'est ce que la carte du bilan hebdo passe naturellement, et Number({…}) donnait NaN → la branche
  // 'fatigue' ne se déclenchait jamais. On tolère les deux formes (source unique = le score).
  const rRaw = o.readiness && typeof o.readiness === 'object' ? o.readiness.score : o.readiness;
  const readiness = rRaw != null && isFinite(Number(rRaw)) ? Number(rRaw) : null;
  let due = false, reason = null;
  if (hardWeeks >= trigger) { due = true; reason = 'accumulation'; }
  else if (readiness != null && readiness < 45 && hardWeeks >= Math.max(3, trigger - 2)) { due = true; reason = 'fatigue'; }
  if (!due) return { due: false, reason: null, hardWeeks, peakZone, peakSets, overMrv, weeks: totals, advice: '', emoji: '' };
  const advice = reason === 'fatigue'
    ? `${hardWeeks} semaines de charge et ta forme baisse : place une semaine de décharge maintenant — coupe le volume de 40 à 50 % (moins de séries), garde tes charges. L'intensité reste, c'est le volume qui chute, et ton corps surcompense.`
    : `${hardWeeks} semaines de charge soutenue d'affilée : c'est l'heure d'une semaine de décharge — coupe le volume de 40 à 50 % (moins de séries, mêmes charges) sur 5-7 jours. Tu récupères, tu surcompenses, tu repars plus fort.`;
  return { due: true, reason, hardWeeks, peakZone, peakSets, overMrv, weeks: totals, advice, emoji: '🧊' };
}
...
const CHANGELOG = [
  ...
  { v: '2.0.281', emoji: '🎓', text: 'Ton suivi de révisions ne te félicite plus « Toutes tes révisions planifiées sont faites » quand tu les as en réalité toutes ratées. La carte « rythme de révision » regarde s’il te reste des séances à faire d’ici l’examen : quand il n’en reste plus, elle affichait « 🎉 Toutes tes révisions faites, chapeau ! » — sauf que « plus rien à venir » ne veut pas dire « tout validé ». Si des séances déjà passées n’ont jamais été cochées, elles ne sont pas faites, elles sont manquées : la carte se contredisait alors frontalement avec le bloc « N révisions en retard » affiché juste en dessous. Désormais elle ne te félicite que si tu as bien tout validé ; sinon elle s’efface et laisse le rappel des révisions en retard porter le vrai message. Rien ne change quand tu es vraiment à jour.' },
  { v: '2.0.280', emoji: '🛡️', text: 'Ta séance guidée ne se trompe plus d’échauffement quand ton titre parle de « contractions ». Elle devine le type de séance (haut du corps, bas, trail…) d’après les mots de ton titre pour te proposer l’échauffement, la prévention blessure et le retour au calme adaptés. Sauf qu’elle repérait le mot « traction » (tractions = tirage, haut du corps) même caché À L’INTÉRIEUR d’un autre mot : un titre comme « Gainage & contractions abdos » contient con·TRACTION·s → la séance recevait à tort l’échauffement, la prévention ÉPAULE (coiffe des rotateurs) et les étirements du HAUT DU CORPS, pour une séance d’abdos. Désormais « traction » n’est reconnu qu’en début de mot : « Tractions » (tes pull-ups) reste bien haut du corps, mais « contractions » ne bascule plus dedans. Même famille de correctif que le « presse à cuisses » traité plus tôt : les mots-clés du titre ancrés sur des bords de mot.' },
  { v: '2.0.279', emoji: '😮‍💨', text: 'On boucle le nettoyage des « ce qui te plombe/porte la tête » du coach focus, cette fois sur le versant NÉGATIF — les jours où ta forme est à plat. Quand ton objectif de deep work est serré mais que ta readiness du matin est au plancher, le coach te rassure déjà : « un cerveau fatigué ne produit pas un vrai bloc, t’acharner empilerait des minutes creuses — un focus court et facile aujourd’hui, soigne ta récup ». Puis il nomme LE frein dominant. Or cette deuxième phrase RE-servait l’appel à l’action : côté sommeil elle finissait par « …pas l’acharnement du jour » (alors que le coach venait de dire « t’acharner… ») ; côté fatigue elle disait « le repos de ce soir vaut plus qu’un bloc forcé maintenant » (déjà porté par « un focus court et facile »). Désormais elle garde sa seule valeur propre — nommer le frein et son mécanisme (« recharge le sommeil, c’est lui qui remettra ton cerveau en état de deep work » ; « ce réservoir vide émousse ta concentration, laisse-le se recharger ») — sans répéter l’injonction déjà posée. Deux redites en moins, le pendant exact des nettoyages 2.0.277/2.0.278.' },
  { v: '2.0.278', emoji: '🧠', text: 'Suite du nettoyage de 2.0.277, cette fois sur les jours SERRÉS. Quand ta semaine de focus est juste-juste mais que ta forme est au vert le matin, le coach t’encourage à pousser (« l’esprit est frais pour tenir un vrai bloc — c’est LE moment de boucler l’objectif »), puis nomme CE QUI porte cette fraîcheur pour que tu répètes la bonne habitude. Côté SOMMEIL, cette deuxième phrase était déjà nickel (« un cerveau reposé est le vrai carburant du deep work, attaque d’abord ta tâche la plus exigeante »). Côté ÉNERGIE, elle RE-servait ce qui venait d’être dit — « l’esprit est vif » (alors que le coach venait de dire « l’esprit est frais ») puis « profite-en pour aller au fond du bloc » (l’injonction « pousse » déjà posée). Désormais elle se cale sur la version sommeil : elle nomme le carburant (« cette vivacité est le carburant du deep work ») et donne le seul conseil neuf — attaquer d’abord la tâche la plus dure tant que la tête suit. Une redite en moins côté énergie.' },
  { v: '2.0.277', emoji: '🧠', text: 'Ton coach « Le focus du moment » ne te répète plus deux fois « prends de l’avance » à la file. Les bons jours (objectif focus dans les temps + tête fraîche le matin), il t’invite d’abord à engranger un bloc d’avance « tant que c’est facile » — puis, juste après, il nomme CE QUI te donne cette clarté (« ta nuit de 8 h », « ton énergie est au top ») pour que tu répètes la bonne habitude. Sauf que cette deuxième phrase RE-servait le même ordre — « … pour engranger un bloc de plus », « … pour banker un bloc d’avance pendant que c’est facile » : le même appel à l’action collé deux phrases de suite. Désormais elle se contente de sa vraie valeur — diagnostiquer le carburant du jour (« c’est ce cerveau reposé qui rend ton bloc si facile, l’habitude à reproduire ») — sans redonder l’invitation déjà posée. Une redite en moins, un coach qui va droit au but.' },
  ...
  { v: '2.0.271', emoji: '🏋️', text: 'La séance guidée ne se contredit plus quand ta forme change en cours de route. Avant : si tu ouvrais ta séance le matin (en forme) puis la reprenais le soir après un check-in devenu difficile (peu de sommeil, courbatures, fatigue), la carte affichait « Récupération fragile : allège la charge » ET, juste en dessous, « 🎯 Cible du jour : monte la charge 💪 » — deux consignes opposées, parce que la cible restait figée sur ta forme du matin. Désormais le conseil de progression ET la cible du jour suivent ta récupération du moment : forme basse à la reprise → on consolide le format en technique propre au lieu de pousser la charge. Un seul message, cohérent.' },
  { v: '2.0.270', emoji: '🎯', text: 'La page Alternance montre enfin ton pipeline d’un coup d’œil. Avant : tes candidatures en liste, plus quelques chiffres (« 3 envoyées · 1 entretien »). Désormais, en haut, une petite barre colorée répartit toutes tes candidatures par étape — à postuler (gris), postulé (bleu), relancé (ambre), entretien (violet), accepté (vert lime) — avec une légende chiffrée en dessous, et les refus comptés à part. Tu vois immédiatement où en est ta recherche : combien attendent une relance, combien sont en entretien, ce qui reste à envoyer. (passe qualité UI, 8/N)' },
  { v: '2.0.269', emoji: '👆', text: 'Les boutons de l’app réagissent mieux au toucher. Avant : quand tu appuyais sur un bouton (Ajouter, Démarrer, ⚙️…), rien ne bougeait — seul le survol le soulevait un peu. Désormais chaque bouton s’enfonce légèrement quand tu le presses (petit effet d’échelle), ce qui donne un vrai retour tactile, et les boutons secondaires voient leur contour passer au vert lime au survol au lieu d’un simple éclaircissement. Un détail, mais c’est ce genre de micro-réactions qui fait qu’une app se sent « vivante » plutôt que figée. (passe qualité UI, 7/N)' },
  { v: '2.0.268', emoji: '📊', text: 'Tes graphes en barres (charge d’entraînement, tonnage, focus, sommeil, révisions, adhérence, volume, 1RM estimé, forme…) ont plus de relief : chaque barre passe d’un aplat de couleur à un léger dégradé vertical — plein en haut, qui s’adoucit vers le bas. Ça donne de la profondeur sans nuire à la comparaison des hauteurs, et c’est raccord avec l’aire dégradée des mini-courbes ajoutée juste avant. Purement visuel : les chiffres et les infobulles ne bougent pas. (passe qualité UI, 6/N)' },
  { v: '2.0.267', emoji: '📉', text: 'Tes mini-courbes (sparklines) ont maintenant un fond dégradé sous la ligne — comme dans les apps de sport et de finance. Avant : un simple trait pour ton tour de taille ou tes nuits de sommeil. Désormais l’aire sous la courbe se remplit d’un dégradé de la couleur de la donnée, qui s’estompe vers le bas : la tendance se lit beaucoup mieux d’un coup d’œil, sans surcharger. La ligne reste nette par-dessus. Purement visuel. (passe qualité UI, 5/N)' },
  { v: '2.0.266', emoji: '📊', text: 'Les trois stats de ton tableau de bord — Vitalité, Focus, Équilibre — ont un look plus soigné. Avant : un simple symbole coloré (♥ ◉ ✦) posé à côté du chiffre. Désormais chaque symbole est dans un petit disque arrondi teinté de SA couleur (rose pour Vitalité, violet pour Focus, vert lime pour Équilibre), le chiffre est un peu plus gros, et le libellé passe en petites capitales espacées — le trio ressemble enfin à de vraies cartes de stats. Purement visuel : les valeurs ne changent pas. (passe qualité UI, 4/N)' },
  { v: '2.0.265', emoji: '🏷️', text: 'Les petits intitulés en majuscules au-dessus de chaque titre de panneau (« AUJOURD’HUI », « TON COACH », « À RATTRAPER »…) et de chaque en-tête de page (« OBJECTIF CORPS », « BIBLIOTHÈQUE »…) retrouvent leur couleur vert lime et leur taille compacte. Ils s’affichaient en gris et un peu trop gros : une règle générique « tout paragraphe dans un panneau est gris » écrasait silencieusement leur style d’accent voulu, aplatissant la hiérarchie sur les 47 panneaux et toutes les pages. Désormais ces intitulés ressortent comme prévu (comme celui au-dessus du logo en haut), ce qui rend chaque section plus lisible d’un coup d’œil. Rien d’autre n’est touché : les descriptions sous les titres restent grises. (passe qualité UI, 3/N)' },
  { v: '2.0.264', emoji: '⚡', text: 'La carte joueur du tableau de bord (Niveau / XP) a gagné son anneau de progression. Avant : ton avatar dans un carré arrondi, avec une barre horizontale plate en dessous pour l’XP. Désormais l’avatar est un rond entouré d’un ANNEAU circulaire vert lime qui se remplit selon ta progression vers le niveau suivant — comme les anneaux d’activité des montres de sport : l’XP se lit d’un coup d’œil, pile là où est ton avatar, et le « NIVEAU X » passe en vert pour ressortir. La barre linéaire redondante disparaît (l’anneau plus le « 40 / 100 XP » chiffré disent déjà tout). Rien ne change au calcul (mêmes XP, mêmes niveaux, même titre) ni à l’accessibilité : l’anneau annonce « niveau X, Y % vers le suivant » au lecteur d’écran. (passe qualité UI, 2/N — après la barre de navigation)' },
  { v: '2.0.263', emoji: '🧭', text: 'La barre de navigation principale a été redessinée. Avant : neuf pastilles de texte serrées sur trois lignes, dont seules trois (Poids, Alternance, Réglages) portaient une icône — les autres non, un détail incohérent qui faisait « bricolé ». Désormais chaque onglet a son icône (🏠 Aujourd’hui, 🗓️ Agenda, 💪 Athlète, ⚖️ Poids, 📚 Exercices, 🥗 Nutrition, 🎯 Focus & vie, 💼 Alternance, ⚙️ Réglages) posée au-dessus de son libellé. Sur téléphone, la nav devient une vraie grille de tuiles lisibles à bordure discrète (au lieu de pastilles transparentes qui se fondaient dans le fond), et l’onglet actif s’éclaire d’un halo vert lime tinté plutôt que d’un aplat vert massif. Sur écran large, la même barre reste une rangée compacte icône + libellé. Rien ne change côté fonctionnement (mêmes onglets, mêmes destinations, focus clavier et lecteur d’écran préservés) — c’est purement l’allure : une nav qui se lit d’un coup d’œil et qui a l’air pensée. (design : premier chantier d’une passe qualité sur toute l’UI, demandée par Adrien)' },
  { v: '2.0.262', emoji: '⚖️', text: 'Ta carte « Coach Poids » ne te dit plus DEUX fois la même chose quand ton poids stagne. Sur un plateau (objectif perte, poids plat depuis au moins deux semaines, cible pas encore atteinte), elle affichait déjà l’ajustement concret « Ton poids stagne (+0 kg/sem sur 16 j). Baisse d’environ 125 kcal/jour ou ajoute du cardio pour relancer la perte. Nouvelle cible : 1875 kcal/j. » — avec le chiffre. Mais juste en dessous, la ligne de tendance ajoutait « 📊 Ta tendance récente ne va pas encore vers la cible — ajuste calories/activité pour t’aligner sur le plan. » : exactement le même message, sans le chiffre, en doublon. Les deux venaient de deux calculs indépendants (rythme sur 6 pesées vs fenêtre datée de 14 jours) rendus côte à côte sans arbitrage. Désormais, dès que l’ajustement calorique s’affiche (il porte déjà le conseil ET le nombre), la ligne de tendance générique se tait. Rien ne change quand l’ajustement n’est pas là (données trop courtes, objectif « maintien »…) : la ligne de tendance reste, comme filet. Et la variante « c’est le signal de ta pause diète » n’est pas touchée. (curation : une seule voix, retirer une redondance vaut mieux qu’ajouter du texte)' },
  ...
  { v: '2.0.244', emoji: '💼', text: 'Ton onglet Alternance ne prend plus un refus pour une offre décrochée. Quand une case de statut disait « Je n’ai pas été pris » (ou « pas été prise »), l’app y voyait le mot « été pris » et classait la candidature en « Accepté 🎉 » — un refus affiché comme une victoire, qui gonflait au passage tes compteurs d’offres acceptées et d’entretiens à chaque synchronisation de ton Google Sheets. La cause : la formule inverse « (pas) été retenu » était déjà bien lue comme un refus, mais sa jumelle « (pas) été pris » ne l’était pas — l’app oubliait le « pas ». Désormais un « pas été pris / pas été prise » est correctement compté comme un refus, exactement comme « pas été retenu ». Les vraies acceptations (« J’ai été pris », « Je suis prise », « Retenu ») ne bougent pas, et une prise de contact en cours (« pas encore pris contact ») reste un dossier en cours, pas un refus.' },
  { v: '2.0.243', emoji: '😴', text: 'Ton bandeau « À rattraper » (en haut du tableau de bord) ne te dit plus « Forme basse — allège aujourd’hui » à partir d’un check-in vieux de plusieurs jours. Tant que tu n’avais pas fait ton check-in du jour, il reprenait ta DERNIÈRE forme enregistrée — même une mauvaise nuit d’il y a une semaine — et l’affichait comme si c’était ta forme d’aujourd’hui, en rouge et prioritaire. Pire, cela contredisait ta carte « Le focus du moment » juste en dessous, qui, elle, ne juge la forme que sur un check-in daté du jour (une readiness d’hier ne dit rien de la forme du jour) : l’une te disait « repose-toi aujourd’hui », l’autre « programme une séance », sur le même écran. Désormais l’alerte « Forme basse » n’apparaît que si tu as un check-in DATÉ D’AUJOURD’HUI, comme le focus. Sans check-in du jour, elle se tait — et un vrai dérèglement de sommeil (nuits courtes + coucher irrégulier sur plusieurs jours) continue, lui, de remonter via son propre signal, qui lit bien tout l’historique. Deux cartes coach qui disent enfin la même chose.' },
  { v: '2.0.242', emoji: '⛰️', text: 'Ta prépa trail ne te dit plus « arrive frais » ET « enchaîne les descentes qui cassent les jambes » en même temps. À l’approche d’une course, la carte « Affûtage » réduit ton volume pour que tu arrives reposé — mais la carte « Descentes » (onglet Ultra) continuait, elle, à te prescrire 1 à 2 séances de descente cassante par semaine jusqu’à J-10 seulement. Or l’affûtage démarre bien plus tôt sur les longues distances (J-11 sur 30 km, J-14 sur marathon, J-18 sur ultra) : il existait donc une fenêtre où les deux conseils se contredisaient, l’un te disant de te reposer, l’autre d’ajouter des dégâts musculaires frais. Désormais la coupe des descentes cassantes est calée sur ta fenêtre d’affûtage (avec un plancher de sécurité à J-10) : dès que tu entres en affûtage, la carte passe à « descentes souples et courtes, jambes fraîches pour le jour J ». Rien ne change loin de la course (le travail de descente spécifique reste au programme des semaines qui précèdent, il protège vraiment tes jambes) — seule la dernière fenêtre avant la course arrête enfin de se contredire. (repeated bout effect, Frontiers Physiol. 2018 · affûtage Bosquet 2007)' },
  ...
  { v: '2.0.221', emoji: '🧭', text: 'Tes deux blocs coach de l’accueil ne se marchent plus sur les pieds. « À rattraper » (ce qui est en retard) et « Le focus du moment » (sur quoi avancer aujourd’hui) étaient calculés chacun dans leur coin : le même sujet pouvait s’afficher DEUX fois, et surtout ils pouvaient se contredire — « allège, ta forme est basse » d’un côté pendant que l’autre te disait « relance ta séance ». Désormais ils s’arbitrent entre eux. Ce que « Le focus du moment » porte déjà est retiré d’« À rattraper » (plus de doublon), et quand ta forme du jour est basse alors que le coach allait te pousser à t’entraîner, la carte se recadre d’elle-même en « Priorité du jour : récupère » — la séance attendra que la forme remonte. Rien d’ajouté à l’écran : une contradiction et un doublon en moins, une seule priorité claire.' },
  { v: '2.0.220', emoji: '⚖️', text: 'Sur ton écran Coach Poids, le plan et le conseil affiché juste à côté ne se contredisent plus sur la DURÉE. Ton plan calorique estimait ta date d’atteinte avec un rythme de perte personnalisé selon ta corpulence (plus tu as de marge, plus il pousse — jusqu’à ~0,9 %/sem — plus tu es sec, plus il ralentit pour protéger ton muscle ; Garthe 2011, Aragon/ISSN 2017), mais le bloc de conseils, lui, calculait encore sa propre estimation sur un rythme fixe de 0,6 %/sem. Résultat : pour un profil corpulent, le plan pouvait annoncer « ≈ 21 semaines » pendant que le conseil disait « ~26 semaines, prévois des pauses » — deux verdicts opposés au même endroit. Les deux tirent désormais leur rythme de la même source, avec les mêmes garde-fous d’élite (ton déficit ne dépasse jamais 25 % de ta dépense, tes calories ne descendent jamais sous ton métabolisme de base) : une seule durée, cohérente, personnalisée par ta corpulence. Rien d’ajouté : une contradiction de plus en moins.' },
  { v: '2.0.219', emoji: '🎯', text: 'Quand ta course objectif approche, « Programme de la semaine » passe tout seul en AFFÛTAGE — la phase qui te fait arriver frais le jour J. Concrètement, sur les ~2 dernières semaines (une seule sur un 10 km, jusqu’à ~2,5 sur un ultra/trail), il RÉDUIT le volume de tes sorties — des courses plus courtes — mais garde le MÊME nombre de séances et ton allure course : c’est exactement ce que montre la grande méta-analyse de référence (Bosquet 2007). Baisser le volume de ~40 à 55 % sans toucher à l’intensité ni à la fréquence améliore la performance de 2 à 4 % ; baisser l’intensité, au contraire, ne sert à rien. La coupe est progressive (légère à deux semaines, plus marquée la semaine de course) et un bandeau t’explique quoi faire. La forme se révèle en réduisant la fatigue, pas en s’entraînant plus fort à la dernière minute.' },
  { v: '2.0.218', emoji: '🫀', text: 'Ta « séance qualité » de course devient une VRAIE séance VO2max — et elle CHANGE chaque semaine au lieu d’être toujours le même tempo. Le coach fait tourner les trois familles de séances dures que font les coureurs d’endurance, toutes centrées sur ton VO2max (le plafond cardio qui fait la vitesse) : les intervalles courts 30/30 (30 s vite / 30 s lent, protocole Billat qui maximise le temps passé haut sans t’épuiser mentalement), les intervalles longs 4×4 « norvégiens » (4 min dur / 3 min footing, le protocole le plus étudié : +7 à 10 % de VO2max en 8 semaines, Helgerud), et les côtes (dur en montée, récup en descente — qui chargent le cardio ET la puissance en protégeant tes jambes). Chaque famille revient un peu plus costaude au tour suivant : c’est de la progression, pas de la répétition. La séance t’explique exactement quoi faire (échauffement, blocs, récup) directement sur la carte.' },
  ...
  { v: '2.0.213', emoji: '✨', text: 'La séance guidée a été redessinée pour être vraiment belle, au niveau du Coach Poids. Tes séries du jour sont maintenant des cartes soignées : un badge rond numéroté, tes chiffres kg/reps en gros et centrés, et un état « Validée ✓ » vert satisfaisant qui illumine la ligne quand tu coches une série. L’exercice en cours devient le héros de l’écran (nom en grand, illustration sur un fond dégradé, format et conseil bien hiérarchisés). Et le chrono de repos passe en vedette : son décompte s’affiche en gros dans un dégradé turquoise→indigo, sur un panneau assorti avec sa barre de progression. Même travail de finition en thème clair comme sombre. Rien ne change au fonctionnement — c’est le plaisir des yeux pendant l’effort qui monte d’un cran.' },
...
// Rythme de perte hebdo SÛR et personnalisé par corpulence — SOURCE UNIQUE partagée par energyPlan
// (le plan « Coach Poids ») et weightTargetAdvice (les conseils affichés juste à côté). Les deux
// dérivaient le rythme chacun de leur côté : energyPlan en 0,5–0,9 %/sem selon l'IMC (depuis 2.0.211),
// weightTargetAdvice resté sur 0,6 % fixe → la durée estimée pouvait DIVERGER sur le même écran (ex. un
// profil corpulent : « ≈ 21 semaines » côté plan vs « ~26 semaines » côté conseil). Même classe de bug
// que les correctifs 2.0.79/2.0.59/2.0.131 (deux verdicts opposés au même endroit). ratePct par IMC
// (Garthe 2011, IJSNEM 21:97 ; Aragon/ISSN 2017, JISSN 14:16) puis DEUX garde-fous d'élite : déficit
// ≤ 25 % du TDEE (ISSN — au-delà, fonte musculaire, adaptation métabolique, chute hormonale) ET apport
// ≥ métabolisme de base. Renvoie le rythme EFFECTIF (kg/sem, après plafonds), le déficit retenu et la
// cible calorique — ou null si données insuffisantes. Pur + testé.
function safeLossRate(weight, bmi, tdee, bmr) {
  const w = Number(weight), b = Number(bmi), t = Number(tdee), m = Number(bmr);
  if (!(w > 0) || !(t > 0) || !(m > 0)) return null;
  const ratePct = b >= 28 ? 0.009 : b >= 25 ? 0.008 : b >= 23 ? 0.007 : b >= 21 ? 0.006 : 0.005;
  let deficit = Math.round(w * ratePct * 7700 / 7);
  deficit = Math.min(deficit, Math.round(t * 0.25));
  const dailyTarget = Math.max(m, t - deficit);
  const eff = t - dailyTarget;
  return { ratePerWeek: Math.round(eff * 7 / 7700 * 100) / 100, deficit: eff, dailyTarget };
}
...
function energyPlan(opts) {
  ...
  if (goal === 'perte') {
    const r = safeLossRate(weight, bmi, tdee, bmr);   // source unique (garde-fous : déficit ≤ 25 % TDEE, apport ≥ BMR)
    ratePerWeek = r.ratePerWeek; deficit = r.deficit; dailyTarget = r.dailyTarget;
  } else if (goal === 'prise') {
...
function attentionDigest(state, todayKey, opts) {
  ...
  // Forme du JOUR uniquement (même politique que adaptiveCoachFocus l.5936, argumentée l.5928-5933) :
  // le message dit « allège AUJOURD'HUI », donc on exige un check-in DATÉ DU JOUR. L'ancien repli sur
  // rec[rec.length - 1] présentait une readiness vieille de plusieurs jours comme la forme du jour — et
  // contredisait frontalement adaptiveCoachFocus, strict sur le même dashboard : une mauvaise nuit d'il
  // y a une semaine y restait muette (readiness=null), mais ici elle criait « Forme basse — allège
  // aujourd'hui » en high (et reframait la journée via coachDayPriority). Sans check-in du jour,
  // l'alerte forme-basse ne se déclenche plus ; le dérèglement de sommeil multi-jours reste couvert par
  // sleepCoachInsight (branche else ci-dessous), qui lit légitimement l'historique, pas la seule nuit.
  ...
  // Sommeil URGENT (court ET coucher irrégulier → sleepCoachInsight tone 'urgent') qui n'a PAS déjà
  // déclenché l'alerte forme-basse ci-dessus (readiness peut rester ≥ 50 quand HRV/courbatures vont
  // bien alors que le rythme, lui, part en vrille). Trou prouvé (§4 ter) : le n°1 d'Adrien — il
  // s'endort vers 6 h — ne vivait QUE dans l'onglet Récupération ; les matins où l'alternance monopolise
  // le focus proactif (cas normal tant qu'il n'a pas postulé du jour), ce signal santé était ABSENT du
  // dashboard (digest vide, focus = alternance). Même clé 'readiness' → dédupliquée contre le focus
  // sommeil (KEY_TO_PILLAR) : n'apparaît QUE quand le focus est ailleurs, jamais en doublon. `else` :
  // au plus UNE alerte readiness (pas de « forme basse » + « sommeil déréglé » le même jour).
  else if (typeof sleepCoachInsight === 'function') {
    const sci = sleepCoachInsight(s.recovery, todayKey);
    if (sci && sci.tone === 'urgent') items.push({ key: 'readiness', emoji: '🌙', text: 'Sommeil déréglé — nuit courte et coucher irrégulier', page: 'athlete', sev: 'high' });
  }
...
function adaptiveCoachFocus(state, todayKey, opts) {
    ...
    if (chosen.pillar === 'sport') {
      ...
      if (g >= 1) {
        ...
        if (wc >= g) {
          ...
          // GARDE-FOU DE TON — le cadrage « aucune obligation de t'y remettre aujourd'hui » n'est cohérent
          // QUE si le pilier n'est PAS présenté comme en recul. En ton `rebuild` l'insight dit juste au-dessus
          // « … s'essouffle … un petit geste suffit à repartir », en `revive` « relance dès aujourd'hui » :
          // y coller « aucune obligation de t'y remettre » se contredit dans le MÊME insight (l'un ordonne de
          // reprendre, l'autre dispense de le faire). Cette contradiction est atteignable dans un cas nominal —
          // la fenêtre glissante 7 j (recentDays < prevDays → rebuild) et la semaine CALENDAIRE (wc ≥ g) sont
          // deux compteurs distincts, l'un peut baisser pendant que l'autre est tenu. Le bonus n'a donc de sens
          // que sous `reinforce` (momentum en hausse, « garde le rythme »), le seul ton où « objectif dans la
          // poche → séance de plus = pur bonus » ne heurte aucune injonction (cas défendu par le commentaire
          // ci-dessus). Le « Objectif hebdo déjà tenu 💪 » (constat, non injonctif) reste valable quel que soit
          // le ton et n'est pas touché.
          const sportDoneToday = (Array.isArray(s.workouts) ? s.workouts : []).some(w => w && w.date === todayKey);
          if (tone === 'reinforce' && !sportDoneToday && typeof readinessScore === 'function') {
            ...
            if (rs && rs.score >= 75) {
              ...
              if (typeof readinessDriver === 'function') {
                ...
                if (drv && (drv.factor === 'sleep' || drv.factor === 'fatigue')) {
                  ...
                  // CURATION §4 ter (#675, sœur de #672 côté branche SERRÉE) : focusGoalFresh (phrases juste
                  // au-dessus) a DÉJÀ dit « l'esprit est frais … c'est LE moment de POUSSER pour boucler
                  // l'objectif ». La variante SOMMEIL de cette note-ci ne le répète pas — sa queue EXPLIQUE le
                  // mécanisme (« un cerveau reposé est le vrai carburant du deep work ») puis donne un ordre
                  // NEUF (séquencer : « attaque d'abord ta tâche la plus exigeante »). La variante ÉNERGIE, elle,
                  // RÉ-servait l'écho : « — l'esprit est vif » (3ᵉ assertion de fraîcheur d'esprit) + « profite-en
                  // pour aller au fond du bloc » (re-service du « pousse … tenir un vrai bloc »). On la calque sur
                  // la variante sommeil : nommer le carburant + le séquençage « le plus dur d'abord », sans redite.
                  ...
                  else insight += ` Et ce qui nourrit cette fraîcheur mentale : ton énergie est au top (fatigue ${drv.value}/5) — cette vivacité est le carburant du deep work, attaque d’abord ta tâche la plus exigeante avant que la journée l’entame.`;
                }
              }
              ...
              if (typeof readinessLimiter === 'function') {
                ...
                if (lim && (lim.factor === 'sleep' || lim.factor === 'fatigue')) {
                  ...
                  // CURATION §4 ter (#678 — pendant NÉGATIF, symétrique de #672/#675 côté positif) : la note
                  // focusGoalDrained JUSTE au-dessus a DÉJÀ porté l'appel à l'action (« un focus court et facile
                  // aujourd'hui, soigne ta récup » + « t'acharner empilerait des minutes creuses »). Ce driver-ci
                  // ne le RÉ-sert plus : sa seule valeur propre est de NOMMER le frein dominant + son mécanisme.
                  // La variante SOMMEIL réservait « pas l'acharnement du jour » (écho de « t'acharner » du parent) ;
                  // la variante ÉNERGIE réservait « le repos de ce soir vaut plus qu'un bloc forcé maintenant »
                  // (écho de « focus court et facile » du parent). On coupe les deux échos, on garde le levier
                  // spécifique (recharger LE sommeil / laisser le réservoir de fatigue se recharger) et le mécanisme.
                  if (lim.factor === 'sleep') insight += ` Et ce qui te plombe la tête aujourd’hui : ta nuit courte de ${String(lim.value).replace('.', ',')} h — recharge le sommeil ce soir, c’est lui qui remettra ton cerveau en état de deep work.`;
                  else insight += ` Et ce qui te plombe la tête aujourd’hui : ta fatigue générale (${lim.value}/5) — c’est ce réservoir vide qui émousse ta concentration, laisse-le se recharger et ton esprit sera bien plus tranchant demain.`;
                }
              }
            } else if (rs && rs.score >= 50) {
            ...
            if (rs && rs.score >= 75) {
              ...
              if (typeof readinessDriver === 'function') {
                ...
                if (drv && (drv.factor === 'sleep' || drv.factor === 'fatigue')) {
                  ...
                  // CURATION §4 ter (#672) : la note d'AVANCE (focusGoalAhead, phrase juste au-dessus) a
                  // DÉJÀ posé l'appel à l'action — « prends de l'avance … tant que c'est facile … un bloc
                  // engrangé maintenant te fait un coussin ». Cette note-ci ne doit donc PAS le RÉPÉTER (elle
                  // le faisait : « … pour engranger un bloc de plus » / « … pour banker un bloc d'avance
                  // pendant que c'est facile », la MÊME injonction deux phrases de suite dans l'insight rendu).
                  // Sa valeur PROPRE (#537) est de NOMMER le moteur pour qu'Adrien le REPRODUISE — on garde ça,
                  // on retire l'ordre en double (retirer vaut mieux qu'ajouter).
                  if (drv.factor === 'sleep') insight += ` Et ce qui te donne cette clarté : ta nuit de ${String(drv.value).replace('.', ',')} h — c’est ce cerveau reposé qui rend ton bloc si facile aujourd’hui, l’habitude à reproduire pour tes prochains bons jours.`;
                  else insight += ` Et ce qui te donne cette clarté : ton énergie est au top (fatigue ${drv.value}/5) — c’est ce ressort mental qui rend ton bloc si facile aujourd’hui, à cultiver pour tes prochains bons jours.`;
                }
              }
            } else if (rs && rs.score < 50) {
  ...
  if (chosen.pillar === 'sommeil' && sleepIns) {
    ...
    // Cohérence insight↔action (§3) : quand le VERDICT certifie « Sommeil solide … rythme régulier »
    // (sleepIns.tone === 'ok' → durée ≥ 7 h ET non irrégulier), prescrire « vise un coucher 30 min plus
    // tôt » se CONTREDIT — le coach félicite un sommeil suffisant puis exige de le corriger comme s'il
    // manquait. Hors plan de recalage, un sommeil 'ok' appelle une action de MAINTIEN (garde le rythme),
    // pas une correction. Les tons 'court'/'attention' (branche else) gardent leur action corrective
    // légitime — court+régulier → « coucher 30 min plus tôt » ; irrégulier → « heure fixe ».
    else if (sleepIns.tone === 'ok') action = 'Rien à corriger côté sommeil : garde cette même heure de coucher ce soir.';
  ...
  // Curation §3 (redondance insight↔action, patron #647 sommeil / #675/#678 drivers focus) : quand
  // fruitGuard a dédié une PHRASE ENTIÈRE au fruit/légume dans l'insight (« Glisse un fruit… » /
  // « Coche la case fruit/légume… »), l'action « cible protéines tenue » ne doit plus RÉ-servir
  // « et un fruit/légume » en queue — sinon la même consigne est dite deux fois dos à dos sur la
  // carte. On ampute la queue → « verrouille l'eau. » seul. Sans fruitGuard, la queue générique
  // reste (aucune phrase dédiée ne la double, c'est le seul effleurement du micronutriment).
  if (fruitGuard && typeof action === 'string') {
    action = action.replace(' et un fruit/légume.', '.');
  }
  ...
  if (chosen.pillar === 'nutrition' && sleepFatLossGuard === null && sleepGainGuard === null
    ...
    // UNE seule phrase (point-virgule, pas point) : orderCoachNotes réordonne PHRASE par phrase, et
    // ce repère est une prémisse NON classée (« forme basse », défaut 4) suivie de sa conclusion
    // CLASSÉE (« tenir l'essentiel : tes protéines… », rang 3 intrants). Deux phrases → la conclusion
    // remontait AU-DESSUS de sa prémisse (le soudage orderCoachNotes ne propage un rang que vers
    // l'AVANT, prémisse-classée→conclusion-défaut, jamais l'inverse), rendant « c'est justement
    // aujourd'hui que… » avant qu'on ait dit POURQUOI. Soudées en une phrase, elles voyagent ensemble.
    ...
    if (typeof focusMinutesTrend === 'function') {
      ...
      if (ft && ft.prev != null) {
        if (ft.dir === 'down' && (tone === 'rebuild' || tone === 'revive')) {
          focusTrend = ft.delta;
          // Garde-fou §4 ter — le pendant, côté NOTE d'insight, du focusRested de #588 (qui ne coupait
          // que l'ACTION). Un jour de tête à plat (focusGoalDrained/focusMarginDrained), l'insight vient
...
// « La priorité du jour » — arbitrage TRANSVERSAL entre les deux surfaces coach du dashboard.
// « À rattraper » (attentionDigest, RÉACTIF) et « Le focus du moment » (adaptiveCoachFocus, PROACTIF)
// lisent le même state au même instant mais ne se connaissent pas : le même pilier peut occuper les
// deux blocs (redondance), et un digest « Forme basse — allège » peut CONTREDIRE un focus proactif
// « relance le sport ». Personne n'optimisait le message CUMULÉ reçu par l'utilisateur (défaut §4 ter
// transposé au niveau carte). Cette fonction PURE réconcilie les deux SANS ajouter de donnée ni de
// champ coach : elle CURATE (retire, dédoublonne, hiérarchise) — pile le périmètre encouragé §3.
//   - `primary`  : l'unique action n°1 du jour + le POURQUOI, prête à rendre dans le bloc focus.
//   - `deduped`  : la liste « À rattraper » débarrassée de ce que porte déjà le focus / le primary.
//   - `defer`    : ce qu'on ASSUME de laisser aujourd'hui (le focus mis de côté quand la santé prime), ou null.
//   - `reframed` : true quand un signal santé (forme basse) a primé sur un focus de momentum sport.
// Règle d'arbitrage (conservatrice, testée) : un item santé `high` (readiness < 50) PRIME sur un focus
// proactif « pousse une séance » (sport, ton rebuild/revive/reinforce) et recadre le ton (repos, pas
// relance). Sinon le focus reste la n°1 (l'alternance, priorité absolue, passe intacte). NON-RÉGRESSION :
// display-only — coachLog reste alimenté par adaptiveCoachFocus, cette fonction ne change JAMAIS quel
// pilier est journalisé (le taux de suivi coachFollowThrough n'est pas faussé). On peut injecter
// opts.focus / opts.digest (tests) ; sinon ils sont calculés. Pur, sans API navigateur. Testé.
function coachDayPriority(state, todayKey, opts) {
  const s = state && typeof state === 'object' ? state : {};
  const o = opts && typeof opts === 'object' ? opts : {};
  const empty = { primary: null, deduped: [], defer: null, reframed: false };
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(todayKey || ''))) return empty;
  const focus = o.focus !== undefined ? o.focus : ((typeof adaptiveCoachFocus === 'function') ? adaptiveCoachFocus(s, todayKey) : null);
  const digest = Array.isArray(o.digest) ? o.digest : ((typeof attentionDigest === 'function') ? attentionDigest(s, todayKey, { cap: o.cap }) : []);
  // Correspondance clé de digest → pilier de focus, pour dédoublonner ce que le focus porte déjà.
  const KEY_TO_PILLAR = { sport: 'sport', readiness: 'sommeil', study: 'etudes', exam: 'etudes', habits: 'habits' };
  const covered = focus && focus.pillar ? focus.pillar : null;
  // Tension santé ↔ momentum : forme basse signalée en réactif VS focus proactif qui pousse une séance.
  // GARDE `!doneToday` (#614) : le recadrage ne prime que sur un focus qui POUSSE une séance. Quand la
  // séance du jour est DÉJÀ faite, le focus sport reinforce n'incite à rien (son action est déjà « séance
  // faite 💪 — verrouille avec des étirements, le reste c'est de la récup »), donc la tension n'existe pas.
  // Recadrer quand même en « récupère, tu relanceras dès que la forme remonte » (a) laissait entendre de
  // reporter un entraînement DÉJÀ fait et (b) écrasait l'acquittement mérité — les deux surfaces restaient
  // cohérentes sans lui (carte « bien méritée » + « À rattraper : forme basse »). Prouvé en rendu chargé §4 ter.
  const restItem = digest.find(d => d && d.key === 'readiness' && d.sev === 'high') || null;
  const sportPush = !!(focus && focus.pillar === 'sport' && !focus.doneToday && (focus.tone === 'rebuild' || focus.tone === 'revive' || focus.tone === 'reinforce'));
  let primary = null, defer = null, reframed = false, primaryKey = null;
  if (restItem && sportPush) {
    // La santé prime : la n°1 devient « récupère », le focus sport est ASSUMÉ comme reporté.
    reframed = true;
    primaryKey = 'readiness';
    primary = {
      source: 'health', pillar: 'recuperation', emoji: restItem.emoji, page: restItem.page,
      headline: 'Priorité du jour : récupère',
      why: `${restItem.text}. Récupérer fait plus progresser que forcer une séance.`,
      action: 'Repos ou séance très douce — tu relanceras l’entraînement dès que la forme remonte.',
    };
    defer = { pillar: focus.pillar, emoji: focus.emoji, page: focus.page, text: `${focus.headline} — ça peut attendre que la forme remonte.` };
  } else if (focus) {
    // Cas normal : le focus (déjà l'unique choix proactif, alternance en tête) EST la priorité du jour.
    primary = { source: 'focus', pillar: focus.pillar, emoji: focus.emoji, page: focus.page, headline: focus.headline, why: focus.insight, action: focus.action };
  } else if (digest.length) {
    // Aucun focus (pas d'historique) : la n°1 est l'item réactif le plus grave.
    const top = digest[0];
    primaryKey = top.key;
...
function weightTargetAdvice(opts) {
  ...
  // Rythme sûr, personnalisé par corpulence — MÊME source (safeLossRate) qu'energyPlan, pour que la
  // durée estimée affichée juste à côté ne DIVERGE plus (cf. safeLossRate). Repli sur ~0,6 %/sem (borné
  // 0,25–0,9 kg) quand le profil (âge) ne permet pas de calculer BMR/TDEE. Prise : 0,25 kg/sem.
  let ratePerWeek;
  if (direction === 'maintien') ratePerWeek = 0;
  else if (direction === 'perte') {
    const bmr = basalMetabolicRate(weight, height, o.age, o.sex);
    const tdee = bmr != null ? Math.round(bmr * (activityLevelFactor(o.activityLevel) || activityFactor(o.sessionsPerWeek))) : null;
    const r = bmr != null ? safeLossRate(weight, weight / (m * m), tdee, bmr) : null;
    ratePerWeek = r ? r.ratePerWeek : Math.min(0.9, Math.max(0.25, Math.round(weight * 0.006 * 100) / 100));
  } else ratePerWeek = 0.25;
...
// Géométrie LISIBLE de la courbe « Prévu vs Réel » du poids (modèle, pas de SVG). Corrige les causes
// d'illisibilité du réel : (1) bornes Y resserrées et arrondies à 0,5 kg autour des valeurs réelles+plan
// (plus d'échelle écrasant le réel), (2) graduations kg pour lire une valeur, (3) `todayX` = position du
// début du plan → sépare le passé RÉEL du futur PRÉVU, (4) coordonnées projetées dans un espace 0-100
// (le rendu superpose des libellés HTML positionnés en %, donc pas de texte déformé). Pur + testé.
function weightForecastModel(planned, actual, opts) {
  const pl = (Array.isArray(planned) ? planned : []).filter(p => p && Number.isFinite(p.value) && /^\d{4}-\d{2}-\d{2}$/.test(p.date));
  ...
  // conteneur en overflow:hidden). Sans réel exploitable, on borne sur le plan pour le montrer.
  const srcVals = act.length >= 2 ? act.map(p => p.value) : all.map(p => p.value);
  let vMin = Math.floor((Math.min(...srcVals) - pad) * 2) / 2, vMax = Math.ceil((Math.max(...srcVals) + pad) * 2) / 2;
  if (vMax - vMin < 1) vMax = vMin + 1;
  const innerH = 100 - topPad - botPad;
  const X = d => (px(d) - xMin) / xr * 100;
  const Y = v => topPad + (vMax - v) / (vMax - vMin) * innerH;
  const proj = arr => arr.map(p => ({ x: +X(p.date).toFixed(2), y: +Y(p.value).toFixed(2), value: p.value, date: p.date }));
  const span = vMax - vMin, step = span <= 2 ? 0.5 : span <= 4 ? 1 : Math.ceil(span / 4);
  const yTicks = [];
...
// deload muscu (`deloadRecommendation`). Un déficit tenu trop longtemps déclenche une ADAPTATION MÉTABOLIQUE
// (baisse du métabolisme de repos, de la thyroïde/leptine et de la NEAT, faim accrue) qui freine la perte ET
// menace le muscle — Trexler 2014 (JISSN 11:7, « metabolic adaptation to weight loss »). Insérer 1-2 semaines
// à la MAINTENANCE (retour à l'équilibre énergétique) restaure ces signaux et protège la masse maigre :
// MATADOR (Byrne 2018, Int J Obes 42:129 — cycles 2 sem déficit / 2 sem maintien : PLUS de gras perdu et
// MOINS de chute du métabolisme de repos qu'un déficit continu) ; ICECAP (Peos 2021, Med Sci Sports Exerc —
// une pause d'1 sem en maintenance a fait GAGNER +0,7 kg de masse maigre chez des pratiquants entraînés).
// Repère pratique (RP/Israetel, Helms) : ~8-12 sem de déficit continu → pause. On mesure le déficit par la
// PERTE DE POIDS réelle (proxy fiable ; pas de saisie calorique quotidienne fidèle) : semaines glissantes,
// poids représentatif = MÉDIANE de la semaine (robuste au bruit eau/sel). La série court tant que le poids
// descend ou plafonne ; une semaine de REGAIN net (une semaine plus récente ≥ 0,3 kg au-dessus d'une plus
// ancienne = un maintien/une pause déjà pris) la casse. Déclenche quand le déficit continu atteint
// `weeksTrigger` semaines calendaires (défaut 10) AVEC une perte cumulée réelle (≥ 1,5 kg) et assez de pesées
// (≥ 4 semaines relevées) pour prouver la continuité. AMBITIEUX MAIS SÛR : n'accélère jamais le déficit —
// propose le geste PROTECTEUR (la pause). goal doit valoir 'perte'. opts : { weeksTrigger, deficit (kcal/j
// retranchés, pour chiffrer le retour au maintien), lookbackWeeks }. Renvoie { due, weeksDeficit, netLossKg,
// weeks:[médianes récentes], kcalBump, advice, emoji } ou null si goal ≠ 'perte' / historique insuffisant
// (< 3 semaines relevées). Display-only, n'alimente aucun compteur. Pur + testé.
function dietBreakRecommendation(weights, goal, todayKey, opts) {
  if (goal !== 'perte') return null;
  const t = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(todayKey || ''));
  if (!t) return null;
  const o = opts && typeof opts === 'object' ? opts : {};
  const trigger = Math.max(4, Math.round(Number(o.weeksTrigger) || 10));
  const lookback = Math.max(trigger + 4, Math.round(Number(o.lookbackWeeks) || 24));
  const today = new Date(+t[1], +t[2] - 1, +t[3]); today.setHours(0, 0, 0, 0);
  // Regroupe les pesées valides par SEMAINE glissante (index 0 = semaine en cours, incomplète).
  const buckets = [];
  for (let i = 0; i <= lookback; i++) buckets.push([]);
  (Array.isArray(weights) ? weights : []).forEach(w => {
    const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(w && w.date || ''));
    if (!m || !Number.isFinite(Number(w.value))) return;
    const d = new Date(+m[1], +m[2] - 1, +m[3]); d.setHours(0, 0, 0, 0);
    const days = Math.round((today - d) / 86400000);
    if (days < 0 || days > lookback * 7 + 6) return;
    buckets[Math.floor(days / 7)].push(Number(w.value));
  });
  const median = arr => { const s = arr.slice().sort((a, b) => a - b); const n = s.length; return n ? (n % 2 ? s[(n - 1) / 2] : (s[n / 2 - 1] + s[n / 2]) / 2) : null; };
  // Semaines COMPLÈTES relevées (indices 1..lookback), de la plus récente vers la plus ancienne.
  const reps = [];
  for (let i = 1; i <= lookback; i++) { const v = median(buckets[i]); if (v != null) reps.push({ wi: i, value: v }); }
  if (reps.length < 3) return null;
  const REGAIN = 0.3;
  // Étend la série tant qu'on ne voit pas un REGAIN net : une semaine plus RÉCENTE dont la médiane dépasse
  // celle d'une plus ANCIENNE de ≥ 0,3 kg = le poids est remonté (maintien/pause déjà pris) → on s'arrête.
  let kEnd = 0;
  for (let k = 1; k < reps.length; k++) {
    if (reps[k - 1].value - reps[k].value >= REGAIN) break;
    kEnd = k;
  }
  const weeksDeficit = reps[kEnd].wi - reps[0].wi;              // durée calendaire de la série (semaines complètes)
  // reps[0] = semaine la plus RÉCENTE (poids le plus bas après la perte), reps[kEnd] = la plus ANCIENNE
  // (poids de départ, plus haut) → perdu = ancien − récent.
  const netLossKg = Math.round((reps[kEnd].value - reps[0].value) * 10) / 10;
  const streakReps = kEnd + 1;
  const weeksArr = reps.slice(0, kEnd + 1).map(r => r.value);
  const deficit = Number(o.deficit);
  const kcalBump = Number.isFinite(deficit) && deficit > 0 ? Math.round(deficit / 25) * 25 : 0;
  const due = weeksDeficit >= trigger && netLossKg >= 1.5 && streakReps >= 4;
  if (!due) return { due: false, weeksDeficit, netLossKg, weeks: weeksArr, kcalBump, advice: '', emoji: '' };
  const advice = `${weeksDeficit} semaines de déficit d'affilée (${netLossKg} kg perdus) : ton corps s'adapte — métabolisme de repos, hormones (leptine, thyroïde) et faim jouent contre toi et la perte ralentit. Place une PAUSE DIÈTE : 1 à 2 semaines à ta maintenance${kcalBump ? ` (≈ +${kcalBump} kcal/jour pour revenir à l'équilibre)` : ''}, protéines hautes et entraînement maintenu. Tu ne reprends pas de gras : tu restaures leptine, thyroïde et NEAT, tu protèges ton muscle, puis tu repars sur un déficit qui remarche.`;
  return { due: true, weeksDeficit, netLossKg, weeks: weeksArr, kcalBump, advice, emoji: '⏸️' };
}
...
function progressionSuggestion(workouts, name, opts) {
  ...
  // Règle 2-for-2 (ACSM 2009, Ratamess ; NSCA Baechle & Earle) : dépasser la cible de ≥ 2 reps signale
  // une charge devenue trop LÉGÈRE → saut de charge plus franc (double incrément) au lieu du micro-pas.
  const overshoot = best.reps >= maxReps + 2;
  const step = overshoot ? inc * 2 : inc;
  const nextLoad = atTop ? Math.round((best.load + step) * 2) / 2 : best.load;
  const nextReps = atTop ? minReps : Math.min(maxReps, best.reps + 1);
  return { action: atTop ? 'weight' : 'reps', nextLoad, nextReps, lastLoad: best.load, lastReps: best.reps, minReps, maxReps, increment: inc, overshoot, date: best.date };
}
// Incrément de charge fondé science (ACSM 2009 : +2-10 % quand on dépasse la cible de 1-2 reps) : plus
// gros sur les poly-articulaires du BAS du corps (+5 kg) que sur le haut du corps / isolation (+2,5 kg).
// Reconnaissance par mots-clés (robuste au texte libre), repli sur exerciseZones. Pur.
function progressionIncrement(name) {
  const t = String(name || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
  if (/\bsquat|souleve|deadlift|\bfente|presse a cuisse|leg press|hip thrust|\bhack\b|jambe|fessier|mollet|cuisse/.test(t)) return 5;
  const zones = (typeof exerciseZones === 'function') ? exerciseZones(name) : [];
  return zones.some(z => z === 'legs' || z === 'glutes') ? 5 : 2.5;
}
...
function progressionText(s) {
  ...
  // Guidance RIR/RPE (Zourdos 2016 ; ACSM 2009) : viser ~1-2 répétitions en réserve (RPE 8-9) plutôt
  // que l'échec — même hypertrophie/force, moins de fatigue et de risque. L'overshoot (2-for-2) justifie
  // un saut de charge plus franc.
  if (s.action === 'weight') return `🎯 ${s.lastReps} reps à ${kg(s.lastLoad)} kg${s.overshoot ? ' (bien au-dessus de la cible)' : ''} : monte à ${kg(s.nextLoad)} kg, repars à ${s.nextReps} reps en gardant ~1-2 reps en réserve (RPE 8-9).`;
  return `🎯 Dernier : ${s.lastReps} reps à ${kg(s.lastLoad)} kg → vise ${s.nextReps} reps à ${kg(s.nextLoad)} kg, ~1-2 reps en réserve (RPE 8-9) ; à ${s.maxReps} reps propres, +${kg(s.increment)} kg.`;
}
// Réconcilie les DEUX surfaces de progression de la séance guidée (le conseil `#guidedProgressionHint`
// et la « Cible du jour » `#guidedTarget`) sous une SEULE autorité — la double progression
// (`progressionSuggestion`, ACSM 2009 / Zourdos 2016), gatée par la récupération. Corrige deux
// contradictions rendues côte à côte : (1) FEU VERT — le conseil prescrivait un naïf « +0,5 kg » tandis
// que la cible affichait « monte la charge » +2,5/5 kg (`progressionIncrement`), chiffres incompatibles ;
// (2) RÉCUP BASSE — le conseil disait « répète le format / allège » pendant que la cible criait quand
// même « monte la charge 💪 ». Sous récup fragile ou séance dure/incomplète (`cautious`), on CONSOLIDE
// (autorégulation : ne pas ajouter de charge quand la forme est basse) ; sinon on suit la cible d'un cran.
// `fallbackHint` = le conseil récup/technique déjà calculé côté rendu (préservé hors feu vert). Pur + testé.
function guidedProgressionLines(sugg, cautious, fallbackHint) {
  const hint = fallbackHint || '';
  if (!sugg || typeof sugg !== 'object') return { hint, target: '' };
  const kg = n => (Math.round(Number(n) * 2) / 2).toString().replace('.', ',');
  if (cautious) {
    return {
      hint,
      target: `🎯 Cible du jour : consolide ${sugg.lastReps} reps × ${kg(sugg.lastLoad)} kg en technique parfaite — tu monteras d’un cran quand la forme sera revenue.`
    };
  }
  return {
    hint: 'Feu vert : suis la cible du jour ci-dessous, un cran à la fois (~1-2 reps en réserve).',
    target: `🎯 Cible du jour : ${sugg.nextReps} reps × ${kg(sugg.nextLoad)} kg${sugg.action === 'weight' ? ' — monte la charge 💪' : ''}`
  };
}
...
function readinessScore(recovery) {
  ...
  // Plafond du LABEL sur le PIRE frein, pas seulement la moyenne. Un frein isolé proche du maximum
  // (courbatures ≥ 4, fatigue ≥ 4, ou nuit courte < 6 h RENSEIGNÉE) est un signal d'autorégulation
  // que le score agrégé peut masquer : à 78/100 avec des courbatures 4/5, dire « Prêt à pousser »
  // contredit directement le conseil « privilégie une séance facile » rendu juste en dessous sur la
  // même carte (app.js #recoveryAdvice). L'autorégulation aiguë se gouverne par son frein limitant,
  // pas par la moyenne (RPE/readiness monitoring — Halson 2014, Sports Med ; DOMS élevés → réduire
  // la charge sur le muscle concerné, Cheung 2003). On borne donc le label à « Correct — garde une
  // marge » tant qu'un frein rouge est levé ; le score CHIFFRÉ, lui, ne bouge pas (toute la logique
  // qui en dépend — charge, décharge, wellness — reste inchangée). Le sommeil n'est un frein que
  // RENSEIGNÉ (sleep > 0), même convention que le score ci-dessus : une donnée absente ne pénalise pas.
  const redFlag = (Number(r.sleep) > 0 && Number(r.sleep) < 6) || fatigue >= 4 || soreness >= 4;
  const label = (score >= 75 && !redFlag) ? 'Prêt à pousser' : score >= 50 ? 'Correct — garde une marge' : 'Récupération prioritaire';
  return { score, label };
}
...
function sleepCoachInsight(recovery, todayKey, opts) {
  ...
  if (week.status === 'court' && irregular) {
    ...
    // Durée correcte mais MOINS de 3 nuits chiffrées (ni durée via `reg`, ni coucher via `useBedtime`) :
    // la régularité n'est PAS encore mesurable. Ne pas AFFIRMER « rythme régulier » qu'on n'a jamais
    // mesuré (§4 ter : un vrai coach ne certifie pas ce qu'il n'a pas vu ; une seule nuit ne dit rien
    // d'un rythme). On salue la durée et on invite à poursuivre les check-ins pour pouvoir juger le rythme.
    tone = 'ok';
...
function sleepImpactReport(state, todayKey, opts) {
  ...
  if (deltas.energy != null && Math.abs(deltas.energy) >= 0.3) {
    verdict = deltas.energy > 0
      ...
      : `Curieux : sur cette période, ton énergie du lendemain a plutôt été meilleure après un coucher tardif (${E.energy}/5 avant ${splitTime} vs ${Lt.energy}/5 après) — d'autres facteurs (durée de sommeil, charge du jour) pèsent sans doute davantage.`;
    ...
    verdict = deltas.focusMin > 0
      ...
      : `Sur cette période, ton focus du lendemain a plutôt été meilleur après un coucher tardif (${E.focusMin} min avant ${splitTime} vs ${Lt.focusMin} après) — d'autres facteurs pèsent sans doute plus ; vise surtout un rythme régulier.`;
...
function sleepPlanDay(plan, recovery, todayKey) {
  ...
  // Progression : fraction du chemin départ→objectif parcourue, référencée sur le coucher réel récent
  // (sinon la planification idéale). Le « départ effectif » prend le PIRE point entre le départ du plan
  // et le coucher réel : ça absorbe une régression (coucher réel plus tardif que le départ) ET le cas
  // dégénéré d'un plan dont le départ est déjà ≤ objectif — sans ce garde-fou, ce dernier forçait
  // progress=100 alors que le coucher réel restait APRÈS l'objectif, soit une barre pleine ET « pas
  // atteint » ET « arrivée dans N jours », trois verdicts contradictoires (miroir du cas `reached`).
  // Pour un plan normal (départ plus tardif que l'objectif), la valeur est identique à l'ancien calcul.
  const fromPos = recentAnchor != null ? recentAnchor : idealAnchor;
  const effShift = Math.max(startAnchor, fromPos) - targetAnchor;   // ≥ 0 : distance du pire point à l'objectif
  const remainPos = Math.max(0, fromPos - targetAnchor);
  const progress = reached ? 100 : (effShift > 0 ? Math.max(0, Math.min(100, Math.round((effShift - remainPos) / effShift * 100))) : 100);
...
// Répartit un volume de course HEBDOMADAIRE (km) sur `count` séances, façon plan de course : la SORTIE
// LONGUE (dernière) prend ~38 % du volume (règle classique 30-40 %, jamais > 50 %), le reste est réparti
// également sur les courses faciles. Sans volume saisi, défaut raisonnable selon l'accent de l'objectif.
// Renvoie [km] (faciles d'abord, longue en dernier), arrondis à 0,5 km, ≥ 3 km. Pur + testé.
function runDistances(count, weeklyKm, emphasis) {
  const n = Math.max(0, Math.min(7, Math.round(Number(count) || 0)));
  if (n === 0) return [];
  const def = { facile: 14, balanced: 22, vitesse: 26 };
  const total = Number(weeklyKm) > 0 ? Number(weeklyKm) : (def[emphasis] || 20);
  const r = k => Math.max(3, Math.round(k * 2) / 2);
  if (n === 1) return [r(Math.min(total, 12))]; // une seule séance : run modéré, pas tout le volume d'un coup
  // La sortie longue prend une plus grosse part quand il y a MOINS de séances (sinon, avec 2 courses,
  // 38 % du volume la rendrait plus COURTE que la facile). Elle reste toujours la séance la plus longue.
  const longFrac = n === 2 ? 0.58 : n === 3 ? 0.42 : 0.35;
  const longKm = Math.max(5, Math.round(total * longFrac));
  const easyEach = (total - longKm) / (n - 1);
  const out = [];
  for (let i = 0; i < n - 1; i++) out.push(r(easyEach));
  out.push(r(longKm));
  return out;
}
// Numéro de semaine ISO (1-53) d'une date 'YYYY-MM-DD' — sert d'index de rotation pour la séance
// qualité (fait tourner le méso-cycle semaine après semaine, sans état persistant). Pur + testé.
function isoWeekNumber(dateKey) {
  ...
  return Math.max(1, Math.ceil(((d - yearStart) / 86400000 + 1) / 7));
}
// Séance QUALITÉ de la semaine — de VRAIS intervalles VO2max, avec VARIÉTÉ et PROGRESSION sur les
// semaines (méso-cycle de 6 semaines qui tourne). Remplace le simple « tempo/seuil » : un coach
// d'endurance fait tourner 3 familles de séances dures, toutes centrées VO2max :
//  · 30/30 (Billat 2000) : 30 s à ~VMA / 30 s footing lent → maximise le temps passé PRÈS de VO2max
//    (~8 min) en restant mentalement tenable ;
//  · 4×4 « Norvégien » (Helgerud 2007) : 4 min à 90-95 % FCmax / 3 min footing → le protocole VO2max
//    le plus étudié, +7-10 % de VO2max en 8 sem ;
//  · Côtes VO2 (60-90 s dur en montée / descente en récup) → charge le cardio ET la puissance en
//    protégeant les jambes (moins d'impact qu'à plat) et affine la foulée.
// La même famille revient PLUS VOLUMINEUSE au tour suivant du cycle (progression bornée). `week` =
// index 1-based (semaine ISO en pratique) ; le modulo fait tourner le méso-cycle. Pur + testé.
function qualitySession(week) {
  const MESO = [
    { key: 'billat3030', reps: 12 },
    { key: 'norv4x4', reps: 4 },
    { key: 'cotes', reps: 6 },
    { key: 'billat3030', reps: 16 }, // progression : +4 répétitions au 2ᵉ tour
    { key: 'norv4x4', reps: 5 },     // progression : +1 bloc de 4 min
    { key: 'cotes', reps: 8 },       // progression : +2 côtes
  ];
  const w = Math.max(1, Math.round(Number(week) || 1));
  const step = MESO[(w - 1) % MESO.length];
  const reps = step.reps;
  if (step.key === 'billat3030') return {
    key: 'billat3030', family: 'VO2max', reps,
    title: `🏃 VO2max · 30/30 (${reps}×)`,
    note: `Micro-intervalles 30/30 : après 15 min d'échauffement, ${reps} × [30 s VITE (allure ~VMA, souffle court, ~1500 m) / 30 s footing lent], puis 10 min retour au calme. Ça maximise le temps passé PRÈS de VO2max en restant tenable (Billat 2000).`,
    source: 'Billat 2000',
  };
  if (step.key === 'norv4x4') return {
    key: 'norv4x4', family: 'VO2max', reps,
    title: `🏃 VO2max · ${reps}×4 min (Norvégien)`,
    note: `Intervalles 4×4 « Norvégiens » : après 15 min d'échauffement, ${reps} × [4 min DUR à 90-95 % de ta FC max (tu ne peux dire que quelques mots) / 3 min footing de récup], puis retour au calme. Le protocole VO2max le plus étudié — +7-10 % en 8 semaines (Helgerud 2007).`,
    source: 'Helgerud 2007',
  };
  return {
    key: 'cotes', family: 'VO2max', reps,
    title: `🏃 VO2max · ${reps}× côtes`,
    note: `Côtes VO2max : après 15 min d'échauffement, trouve une montée de 60-90 s et fais ${reps} × [montée à effort DUR / descente en trottinant]. Ça charge le cardio ET la puissance en protégeant tes jambes (moins d'impact qu'à plat) et affine ta foulée.`,
    source: 'côtes VO2max',
  };
}
function buildTrainingWeek(zones, strengthDays, runs, sameDay, opts) {
  ...
  const o = opts || {};
  // AFFÛTAGE : une course objectif approche → on réduit le VOLUME (km/durée) en GARDANT la FRÉQUENCE
  // (même nb de courses) et l'INTENSITÉ (la séance qualité reste au programme) — cf. taperPlan / Bosquet 2007.
  const taper = taperPlan(o.raceDaysLeft, o.raceKm) || null;
  let dist = runs > 0 ? runDistances(runs, o.weeklyKm, o.emphasis) : [];
  if (taper && dist.length) dist = dist.map(k => Math.max(3, Math.round(k * taper.volumeMul * 2) / 2));
  const qualityIdx = runs >= 3 ? Math.max(1, Math.floor((runs - 1) / 2)) : -1; // un run du milieu, pas la longue
  const qs = qualityIdx >= 0 ? qualitySession(o.week) : null; // VO2max qui tourne semaine après semaine
  const runSessions = [];
  for (let i = 0; i < runs; i++) {
    ...
    const km = dist[i];
    const base = quality ? qs.title : long ? '🏃 Sortie longue' : '🏃 Course facile';
    ...
    runSessions.push({ type: 'run', kind: 'sport', long, quality, intensity, km, note, session: quality ? qs : null, title: km ? `${base} · ${String(km).replace('.', ',')} km` : base, exercises: [] });
  }
  ...
  if (sameDay) {
    ...
    return { zones, strengthDays, runs, sameDay: true, sessions: days.length, days, taper };
  }
  ...
  return { zones, strengthDays, runs, sameDay: false, sessions: days.length, days, taper };
}
