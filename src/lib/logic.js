...
// Victoires notées lors des rituels du soir (`reflections[].win`), les plus récentes d'abord.
// Ces victoires étaient conservées (jusqu'à 180 jours) mais jamais réaffichées : seul le champ de
// saisie du jour était repeuplé. Exclut le jour courant et les entrées vides.
// opts : { cap = 5, days = 90 }. Renvoie [{ date, win, daysAgo }]. Pur + testé.
function recentWins(reflections, todayKey, opts) {
  ...
  return (Array.isArray(reflections) ? reflections : [])
    .filter(r => r && isKey(r.date) && r.date < todayKey && String(r.win || '').trim())
    .map(r => ({ date: r.date, win: String(r.win).trim().slice(0, 120), daysAgo: daysUntil(r.date, todayKey) }))
