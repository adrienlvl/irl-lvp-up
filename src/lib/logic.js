...
function taperPlan(daysLeft, raceKm) {
  ...
  // Durée d'affûtage selon la distance (Bosquet ~2 sem ; échelonnée par distance, Mujika 2003).
  const taperDays = km <= 12 ? 7 : km <= 30 ? 11 : km <= 45 ? 14 : 18;
...
// pas dans les ~10 derniers jours (la protection se construit tôt ; on n'ajoute pas de dommage frais
// avant le jour J : cohérent avec `taperPlan`).
...
function downhillPrep(dplusPerWeek, raceDaysLeft, raceKm) {
  ...
  // À ~10 jours de la course : STOP aux descentes cassantes — la protection est déjà acquise, on
  // préserve la fraîcheur (même logique que l'affûtage : pas de dommage frais avant le jour J).
  if (hasRace && d <= 10) {
    return {
      sessionsPerWeek: 0, window: 'race', source,
      protocol: `À J-${d}, plus de séance de descente « cassante » : la protection excentrique est déjà en place. Garde des descentes souples et courtes, jambes fraîches pour le jour J.`,
