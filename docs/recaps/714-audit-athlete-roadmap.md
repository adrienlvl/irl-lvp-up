...
| Rang | Chantier | Geste |
|---|---|---|
| A1 | **D1 — tuer le double programme** | `program-panel` (2 159 px) est absorbé par le Plan de bataille : ses séances suggérées deviennent des variantes du générateur unifié, son bouton « Démarrer guidée » existe déjà dans le Plan. C'est le plus gros panneau redondant de l'app. Lié à la décision d'octobre sur le markup masqué (les 3 générateurs) — l'avancer. |
