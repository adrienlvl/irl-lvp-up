...
| ~~B2~~ | ~~**La mémoire par exercice**~~ — **DÉJÀ LIVRÉE, constaté à la 124.** `memoireForceParCadence` rend « Ta force progresse le plus à 3,5 séances/semaine : +17,5 kg en moyenne par exercice suivi, contre +9 kg à 2,5. **Squat y a pris le plus (+22,5 kg)** » — soit l'exemple de cette ligne, avec le contrefactuel et la réserve. Le détail par exercice × cadence existe (`parCadence`) mais reste tu : **2 mesures par cadence** ne portent pas une affirmation par exercice. | — |
| ~~B3~~ | ~~**Mensurations lues**~~ — **FAIT (124), mais pas comme la ligne l'annonçait.** Tendance, delta récent, courbe et historique étaient déjà peints. Le vrai défaut était à côté : `recompositionInsight` comparait un delta de poids sur tout l'historique des pesées à un delta de taille sur tout celui des mensurations. Mesuré : 2 ans de balance (−12 kg) face à 3 mois de ruban (−1,5 cm) → « perte de gras bien engagée », alors que la fenêtre commune disait « recomposition ». `fenetreRecomposition` borne les deux séries à leur départ commun, exige 2 points de chaque côté, et l'écran nomme sa période. **Revu à la 126** : la fenêtre n'était fermée qu'au DÉPART — un ruban muet depuis 60 jours donnait « Sur les 6 derniers mois : −2 cm » pour des centimètres perdus entre janvier et mai. Elle se ferme désormais aux deux bouts, et l'écran dit jusqu'où il a regardé quand la dernière mensuration est ancienne. | — |
...
| 8 | ~~**B2, B3**~~ | **fait (124)** — B3 corrigée sur la fenêtre commune, B2 constatée déjà livrée |
