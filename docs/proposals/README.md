...
| [`robustesse-classificateurs-import-alternance.md`](robustesse-classificateurs-import-alternance.md) | Robustesse (module Alternance sacré) | ⏳ **à trancher** — audit de **10 fragilités prouvées** (toutes exécutées) du classifieur/import du Sheets d'alternance : 5 GRAVES (`pas le candidat retenu`→accepté, `pas encore contacté`→postulé, virgule dans un champ `;`→colonnes décalées, ids en collision à l'import en lot, dates FR vidées). Reco : **A** (le VPS corrige les 5 cas **sans ambiguïté** en étapes autonomes avec test échoue-avant/passe-après ; les 4 choix de **design** — clé de fusion, refus vs accepte, décline, format US — attendent Adrien car ils reclassent des cellules réelles au sync). 5 décisions en fin de doc. |
| [`dates-impossibles-garde-ingestion.md`](dates-impossibles-garde-ingestion.md) | Robustesse (dette « dates n°1 ») | ⏳ **à trancher** — la « dette dates n°1 » corrigée 3× en réactif (#671/#676/#692) est **encore ouverte** : `normalizeExamGoal` (`logic.js:1911`) valide la date d'épreuve par **format seul** → `2026-02-30` alimente `daysUntil`→`new Date` → **compte à rebours d'examen faux** (déborde au 2 mars). Reco : **A** (garde canonique `isRealDateKey` aux 2 sites prouvés — études + import GLC — en étapes autonomes A.1/A.2, dès que la rotation rouvre `robustesse`) ; **B** = uniformisation totale + retrait de `isBoundedDateKey` (surface de régression plus large). 4 décisions en fin de doc. |
