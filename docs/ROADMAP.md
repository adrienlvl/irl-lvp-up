...
## 📍 État actuel — build 1.8.6 (2026-07-09)

App **100 % locale**, hors accès réseau **opt-in** (auto-update GitHub · sync agenda par URL · trajet auto OpenStreetMap, allowlist stricte). **117 tests + smoke** verts. Livré au-delà de la roadmap initiale (boucles #36→63) :
...
- **Coaching** : **47 exercices** (vraie photo d'humain pour **les 47**, 8 planches) tous programmés ; générateur de repas frigo+envie + liste de courses (CIQUAL, cuit avant cru).
- **Confort/infra** : densité, retour-en-haut, version affichée, auto-update.
- ⚠️ Versions **1.5.2 → 1.8.6 non publiées** sur GitHub Releases (attendent un `npm run release` d'Adrien).
