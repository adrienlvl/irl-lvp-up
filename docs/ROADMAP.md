...
## 📍 État actuel — build 1.9.6 (2026-07-09)

App **100 % locale**, hors accès réseau **opt-in** (auto-update GitHub, re-check /3 h · sync agenda par URL · trajet auto OpenStreetMap, allowlist stricte). **124 tests + smoke** verts. Livré au-delà de la roadmap initiale (boucles #36→72) :
...
- ⚠️ Versions **1.5.2 → 1.9.6 non publiées** sur GitHub Releases (attendent un `npm run release` d'Adrien).
- ⏳ **Connexions sportives (Strava / Garmin / Polar)** demandées → menu Paramètres + OAuth : **Vague S.8** (nécessite qu'Adrien enregistre une app développeur chez chaque service ; Strava = faisable en solo, Garmin/Polar = API partenaires gated).
