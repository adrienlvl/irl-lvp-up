...
## 📍 État actuel — build 1.8.8 (2026-07-09)

App **100 % locale**, hors accès réseau **opt-in** (auto-update GitHub · sync agenda par URL · trajet auto OpenStreetMap, allowlist stricte). **120 tests + smoke** verts. Livré au-delà de la roadmap initiale (boucles #36→65) :
...
- **Coaching** : **47 exercices** (vraie photo d'humain pour **les 47**, 8 planches) tous programmés, **filtrables par objectif physique** (abdos, bras, jambes, dos, pecs, épaules, fessiers) ; générateur de repas frigo+envie + liste de courses (CIQUAL, cuit avant cru).
- **Confort/infra** : densité, retour-en-haut, version affichée, auto-update.
- ⚠️ Versions **1.5.2 → 1.8.8 non publiées** sur GitHub Releases (attendent un `npm run release` d'Adrien).
...
- [x] **5.9** **Objectifs physiques par zone** (demande d'Adrien : « objectif 6-pack, gros biceps, grosses jambes… ») — sélecteur **🎯 Objectif** dans la bibliothèque : Abdos 🔥 · Bras 💪 · Pectoraux 🎯 · Dos 🦅 · Épaules 🏔️ · Jambes 🦵 · Fessiers 🍑. Les 47 exercices sont tagués par **zone musculaire** (`EXERCISE_ZONES`, zone principale d'abord) ; choisir un objectif filtre la bibliothèque et **classe les exercices les plus ciblés en tête** (`goalMatch`/`goalRank` purs + testés). Test de couverture : chaque objectif ≥ 5 exercices, chaque exercice tagué. ✅ _boucle #64 (build 1.8.7)._
  - [~] **Animer l'exécution** (Adrien : « je veux TOUTES les animer »). **Moteur prêt & validé** (boucle #65, build 1.8.8) : `buildAnimatedArt` empile 2 calques photo (position A + position B) et fait clignoter la position B en alternance via CSS (`@keyframes exFrameFlip`, respecte `prefers-reduced-motion`). Activé par planche : dès qu'une planche « position B » (`sheet-<n>b`) est livrée, son numéro entre dans `ANIMATED_SHEETS` et ses exercices s'animent en vue détail/séance. Bascule vérifiée par capture 2 frames. **En attente d'Adrien** : générer **8 planches « position 2 »** (mêmes exercices/cases que v1..v8) — plan case par case fourni. On réutilise les 8 planches actuelles comme « position 1 » → 8 planches à générer, pas 16.
