...
## 📍 État actuel — build 1.8.7 (2026-07-09)

App **100 % locale**, hors accès réseau **opt-in** (auto-update GitHub · sync agenda par URL · trajet auto OpenStreetMap, allowlist stricte). **119 tests + smoke** verts. Livré au-delà de la roadmap initiale (boucles #36→64) :
...
- ⚠️ Versions **1.5.2 → 1.8.7 non publiées** sur GitHub Releases (attendent un `npm run release` d'Adrien).
...
- [x] **5.9** **Objectifs physiques par zone** (demande d'Adrien : « objectif 6-pack, gros biceps, grosses jambes… ») — sélecteur **🎯 Objectif** dans la bibliothèque : Abdos 🔥 · Bras 💪 · Pectoraux 🎯 · Dos 🦅 · Épaules 🏔️ · Jambes 🦵 · Fessiers 🍑. Les 47 exercices sont tagués par **zone musculaire** (`EXERCISE_ZONES`, zone principale d'abord) ; choisir un objectif filtre la bibliothèque et **classe les exercices les plus ciblés en tête** (`goalMatch`/`goalRank` purs + testés). Test de couverture : chaque objectif ≥ 5 exercices, chaque exercice tagué. ✅ _boucle #64 (build 1.8.7)._
  - ⏳ **À venir — animer l'exécution** (Adrien : « les photos sont animées ? »). Aujourd'hui chaque photo est **une image fixe** (une pose). Pour un vrai mini-mouvement animé avec un humain, il faut **2 poses par exercice** (début ↔ fin) à faire alterner en CSS (pas de nouveau moteur, juste 2 cases). Nécessite qu'Adrien génère des planches « 2 poses » (prompt à fournir). Alternative immédiate possible : rejouer la **figure SVG animée** en superposition optionnelle (mais Adrien préfère l'humain).
